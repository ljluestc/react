'use strict';/* eslint-disable no-for-of-loops/no-for-of-loops */const getComments = require('./getComments');function transform(babel) {
  const {types: t} = babel;  // A very stupid subset of pseudo-JavaScript, used to run tests conditionally
  ...
  // Output:
  //te a && (b || c)
  //st_gate(ctx => ctx.a && (ctx.b || ctx.c), 'some test', () => {/*...*/});
  //
...
const tokens = tokenize(code);let i = 0;
...
st condition = buildGateCondition(comments);
(condition !== null) {
callee.name =
