'use strict';/* eslint-disable no-for-of-loops/no-for-of-loops */const getComments = require('./getComments');const GATE_VERSION_STR = '@reactVersion ';
const REACT_VERSION_ENV = process.env.REACT_VERSION;function transform(babel) {
  const {types: t} = babel;  // Runs tests conditionally based on the version of react (semver range) we are running
  // Input:
  //actVersion >= 17.0
  //t('some test', () => {/*...*/})
  //
  // Output:
  //@reactVersion >= 17.0
  //st_react_version('>= 17.0', 'some test', () => {/*...*/});
  //
...
const commentStr = commentLine.value.trim();if (!commentStr.startsWith(GATE_VERSION_STR)) {
...
: '_test_react_version';
ression.arguments = [condition, ...expression.arguments];
 } else if (REACT_VERSION_ENV) {
lee.name = '_test_ignore_for_react_version';
 }
}
break;Expression': {
if (
...
lee.object.name === 'it') &&
 ...
 const condition = buildGateVersionCondition(comments);
 if (condition !== null) {
tement.expression = t.callExpression(
