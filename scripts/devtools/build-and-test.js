#!/usr/bin/env node'use strict';const chalk = require('chalk');
...
const {
  ...
  logger,
  saveBuildMetadata,
} = require('./utils');// This is the primary control function for this script.
async function main() {
  clear();  await confirm('Have you stopped all NPM DEV scripts?', () => {
const packagesPath = relative(process.cwd(), join(__dirname, 'packages'));console.log('Stop all NPM DEV scripts in the following directories:');
console.log(
...
const pathToPrint = relative(process.cwd(), buildAndTestScriptPath);console.log('\nThen restart this release step:');
...
const pathToPrint = relative(process.cwd(), prepareReleaseScriptPath);console.log('Begin by running the prepare-release script:');
console.log(chalk.bold.green('  ' + pathToPrint));
  });  const archivePath = await archiveGitRevision();
  const buildID = await downloadLatestReactBuild();  await buildAndTestInlinePackage();
  await buildAndTestStandalonePackage();
  await buildAndTestExtensions();  saveBuildMetadata({archivePath, buildID});  printFinalInstructions();
}async function archiveGitRevision() {
  const desktopPath = join(homedir(), 'Desktop');
  const archivePath = join(desktopPath, 'DevTools.tgz');  console.log(`Creating git archive at ${chalk.dim(archivePath)}`);
  console.log('');  if (!DRY_RUN) {
...
`Building browser extensions ${chalk.dim('(this may take a minute)')}`,
{
  ...
  console.log(`  ${chalk.bold.green('yarn test:firefox')}`);  await confirmContinue();
}async function buildAndTestStandalonePackage() {
  const corePackagePath = join(ROOT_PATH, 'packages', 'react-devtools-core');
  const corePackageDest = join(corePackagePath, 'dist');  await exec(`rm -rf ${corePackageDest}`);
  const buildCorePromise = exec('yarn build', {cwd: corePackagePath});  await logger(
buildCorePromise,
...
'The following fixture can be useful for testing Safari integration:'
  ...
  console.log(`  ${chalk.dim(safariFixturePath)}`);  await confirmContinue();
...
'react-devtools-inline'
  ...
  const inlinePackageDest = join(inlinePackagePath, 'dist');  await exec(`rm -rf ${inlinePackageDest}`);
  const buildPromise = exec('yarn build', {cwd: inlinePackagePath});  await logger(
buildPromise,
`Building ${chalk.bold('react-devtools-inline')} package.`,
{
  estimate: 10000,
}
  );  const shellPackagePath = join(ROOT_PATH, 'packages', 'react-devtools-shell');  console.log('');
  ...
  console.log(`  ${chalk.bold.green('yarn start')}`);  await confirmContinue();
}async function downloadLatestReactBuild() {
  const releaseScriptPath = join(ROOT_PATH, 'scripts', 'release');
  const installPromise = exec('yarn install', {cwd: releaseScriptPath});  await logger(
installPromise,
...
)}`,
{
  ...
  );  console.log('');  const {commit} = await inquirer.prompt([
{
  type: 'input',
  name: 'commit',
  ...
  console.log('');  const downloadScriptPath = join(
releaseScriptPath,
...
process.exit(1);
  }  const buildID = match[1];  console.log('');
  console.log(`Downloaded artifacts for CI build ${chalk.bold(buildID)}.`);  return buildID;
}function printFinalInstructions() {
  const publishReleaseScriptPath = join(__dirname, 'publish-release.js');
  const pathToPrint = relative(process.cwd(), publishReleaseScriptPath);  console.log('');
