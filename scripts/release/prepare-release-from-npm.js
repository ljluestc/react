#!/usr/bin/env node'use strict';const {join} = require('path');
const {getPublicPackages, handleError} = require('./utils');const checkOutPackages = require('./prepare-release-from-npm-commands/check-out-packages');
...
const theme = require('./theme');const run = async () => {
  try {
const params = parseParams();
params.cwd = join(__dirname, '..', '..');const isExperimental = params.version.includes('experimental');if (!params.version) {
  params.version = await getLatestNextVersion();
}params.packages = await getPublicPackages(isExperimental);// Map of package name to upcoming stable version.
...
const versionsMap = new Map();if (isExperimental) {
  console.error(
...
await updateStableVersionNumbers(params, versionsMap);if (!params.skipTests) {
