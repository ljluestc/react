#!/usr/bin/env node'use strict';const {readFileSync, writeFileSync} = require('fs');
const {readJson, writeJson} = require('fs-extra');
const {join} = require('path');const run = async ({cwd, packages, skipPackages, tags}) => {
  if (!tags.includes('latest')) {
// Don't update version numbers for alphas.
return;
  }  const nodeModulesPath = join(cwd, 'build/node_modules');
  const packagesPath = join(cwd, 'packages');  // Update package versions and dependencies (in source) to mirror what was published to NPM.
...
sourcePackageJSON.peerDependencies = publishedPackageJSON.peerDependencies;await writeJson(sourcePackageJSONPath, sourcePackageJSON, {spaces: 2});
