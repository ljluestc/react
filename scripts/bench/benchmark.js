'use strict';const Lighthouse = require('lighthouse');
const chromeLauncher = require('chrome-launcher');const stats = require('stats-analysis');
const config = require('lighthouse/lighthouse-core/config/perf-config');
const spawn = require('child_process').spawn;
const os = require('os');const timesToRun = 10;function wait(val) {
...
means.push(stats.mean(bootstrap(data)));
  ...
  const data = [];
  const averages = [];  runs.forEach((entries, x) => {
...
averages[i].sem = calculateStandardErrorOfMean(data[i]);
  ...
  const platform = os.platform();  if (platform === 'linux') {
...
runs: [],
averages: [],
  };  await initChrome();  for (let i = 0; i < timesToRun; i++) {
let chrome = await launchChrome(headless);results.runs.push(await runScenario(benchmark, chrome));
