'use strict';const {readdirSync, statSync} = require('fs');
...
const serveBenchmark = require('./server');function getBenchmarkNames() {
...
const skipBuild = argv['skip-build'];async function runBenchmarks(reactPath) {
  const benchmarkNames = getBenchmarkNames();
  const results = {};
  const server = serveBenchmark();
  await wait(1000);  for (let i = 0; i < benchmarkNames.length; i++) {
const benchmarkName = benchmarkNames[i];if (
!benchmarkFilter ||
...
async function benchmarkRemoteMaster() {
  ...
  let commit = argv.remote;  if (!commit || typeof commit !== 'string') {
...
chalk.green.bold('Local (Current Branch)')
  ...
  const localResults = await benchmarkLocal(join(__dirname, '..', '..'));  if (showResults) {
...
chalk.yellow.bold('Remote (Merge Base)')
  ...
  const remoteMasterResults = await benchmarkRemoteMaster();  if (showResults) {
