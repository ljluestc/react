 ...
 *
 * @emails react-core
 */'use strict';const path = require('path');const {ESLint} = require('eslint');function getESLintInstance(format) {
  return new ESLint({
useEslintrc: false,
overrideConfigFile:
...
async function lint(folder) {
  ...
  const eslint = esLints.cjs;  const results = await eslint.lintFiles([
