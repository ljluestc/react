...
// and ReactElementJSX-test.jest.mock('react/jsx-runtime', () => require('./jsx-runtime'), {virtual: true});
...
RequiredPropComponent.propTypes = {prop: PropTypes.string.isRequired};it('works', () => {
  const container = document.createElement('div');
...
expect(() => (element.type = 'div')).not.toThrow();
  ...
  expect(element.props.children).toBe(undefined);  const element2 = React.cloneElement(
...
render() {
  ...
  expect(React.isValidElement(<Component />)).toEqual(true);  expect(React.isValidElement(null)).toEqual(false);
  ...
  Component.defaultProps = {fruit: 'persimmon'};  const container = document.createElement('div');
  const instance = ReactDOM.render(<Component fruit="mango" />, container);
  expect(instance.props.fruit).toBe('mango');  ReactDOM.render(<Component />, container);
...
render() {
  ...
  NormalizingComponent.defaultProps = {prop: 'testKey'};  const container = document.createElement('div');
  const instance = ReactDOM.render(<NormalizingComponent />, container);
  expect(instance.props.prop).toBe('testKey');  const inst2 = ReactDOM.render(
...
next: function () {
  ...
  iterable.entries = iterable['@@iterator'];  ReactTestUtils.renderIntoDocument(<Component>{iterable}</Component>);
...
{withoutStack: true}
  ...
  RequiredPropComponent.defaultProps = {prop: null};  expect(() =>
...
render() {
  ...
  const innerObj = {};
  const outerObj = {};  class Wrapper extends React.Component {
getObject = () => {
...
getObject = () => {
  ...
  }  let mounted = false;  class Component extends React.Component {
render() {
  const inner = (
