...
import {BORDER_SIZE, COLORS, SUSPENSE_EVENT_HEIGHT} from './constants';const HEIGHT = SUSPENSE_EVENT_HEIGHT; // TODO Constant name
const ROW_WITH_BORDER_HEIGHT = HEIGHT + BORDER_SIZE;const BASE_URL_REGEX = /([^:]+:\/\/[^\/]+)/;export class NetworkMeasuresView extends View {
  _depthToNetworkMeasure: Map<number, NetworkMeasure[]>;
  _hoveredNetworkMeasure: NetworkMeasure | null = null;
  _intrinsicSize: IntrinsicSize;
  _maxDepth: number = 0;
  _profilerData: TimelineData;  onHover: ((event: NetworkMeasure | null) => void) | null = null;  constructor(surface: Surface, frame: Rect, profilerData: TimelineData) {
super(surface, frame);this._profilerData = profilerData;this._performPreflightComputations();
  }  _performPreflightComputations() {
this._depthToNetworkMeasure = new Map();const {duration, networkMeasures} = this._profilerData;networkMeasures.forEach(event => {
const depth = event.depth;this._maxDepth = Math.max(this._maxDepth, depth);if (!this._depthToNetworkMeasure.has(depth)) {
...
receiveResponseTimestamp || firstReceivedDataTimestamp || timestampEnd;// Convert all timestamps to x coordinates.
...
const xStop = timestampToPosition(timestampEnd, scaleFactor, frame);const width = durationToWidth(xStop - xStart, scaleFactor);
if (width < 1) {
return; // Too small to render at this zoom level
}baseY += depth * ROW_WITH_BORDER_HEIGHT;const outerRect: Rect = {
origin: {
x: xStart,
y: baseY,
},
...
: '';const label = durationLabel + displayUrl;drawText(label, context, outerRect, visibleArea);
...
const hoverTimestamp = positionToTimestamp(location.x, scaleFactor, frame);const adjustedCanvasMouseY = location.y - frame.origin.y;
const depth = Math.floor(adjustedCanvasMouseY / ROW_WITH_BORDER_HEIGHT);
const networkMeasuresAtDepth = this._depthToNetworkMeasure.get(depth);const duration = this._profilerData.duration;if (networkMeasuresAtDepth) {
...
const timestampEnd =
finishTimestamp || lastReceivedDataTimestamp || duration;if (
