...
import {
  ...
  TOP_ROW_PADDING + REACT_EVENT_DIAMETER + TOP_ROW_PADDING;export class SchedulingEventsView extends View {
  _profilerData: TimelineData;
  _intrinsicSize: Size;  _hoveredEvent: SchedulingEvent | null = null;
  onHover: ((event: SchedulingEvent | null) => void) | null = null;
  onClick:
| ((event: SchedulingEvent | null, eventIndex: number | null) => void)
| null = null;  constructor(surface: Surface, frame: Rect, profilerData: TimelineData) {
super(surface, frame);
...
const {frame} = this;
const {timestamp, type, warning} = event;const x = timestampToPosition(timestamp, scaleFactor, frame);
...
return; // Not in view
}let fillStyle = null;if (warning !== null) {
...
frame,
);const highlightedEvents: SchedulingEvent[] = [];schedulingEvents.forEach(event => {
