...
import {ColorGenerator, dimmedColor, hslaColorToString} from './utils/colors';// Source: https://source.chromium.org/chromium/chromium/src/+/master:out/Debug/gen/devtools/timeline/TimelineUIUtils.js;l=2109;drc=fb32e928d79707a693351b806b8710b2f6b7d399
const colorGenerator = new ColorGenerator(
  {min: 30, max: 330},
  {min: 50, max: 80, count: 3},
  85,
);
colorGenerator.setColorForID('', {h: 43.6, s: 45.8, l: 90.6, a: 100});function defaultHslaColorForStackFrame({scriptUrl}: FlamechartStackFrame) {
...
COLOR_HOVER_DIM_DELTA,
  ...
  /** Layer to display */
  _stackLayer: FlamechartStackLayer;  /** A set of `stackLayer`'s frames, for efficient lookup. */
  _stackFrameSet: Set<FlamechartStackFrame>;  _internalModuleSourceToRanges: InternalModuleSourceToRanges;  _intrinsicSize: Size;  _hoveredStackFrame: FlamechartStackFrame | null = null;
  _onHover: ((node: FlamechartStackFrame | null) => void) | null = null;  constructor(
surface: Surface,
frame: Rect,
...
_intrinsicSize,
visibleArea,
} = this;context.fillStyle = COLORS.PRIORITY_BACKGROUND;
context.fillRect(
visibleArea.origin.x,
visibleArea.origin.y,
visibleArea.size.width,
visibleArea.size.height,
);const scaleFactor = positioningScaleFactor(_intrinsicSize.width, frame);for (let i = 0; i < _stackLayer.length; i++) {
const stackFrame = _stackLayer[i];
const {name, timestamp, duration} = stackFrame;const width = durationToWidth(duration, scaleFactor);
if (width < 1) {
...
}const showHoverHighlight = _hoveredStackFrame === _stackLayer[i];let textFillStyle;
...
const {timestamp, duration} = flamechartStackFrame;const x = Math.floor(timestampToPosition(timestamp, scaleFactor, frame));
...
startIndex = currentIndex + 1;
  ...
  }  _didGrab: boolean = false;  handleInteraction(interaction: Interaction, viewRefs: ViewRefs) {
...
break;
  ...
  _flamechartRowViews: FlamechartStackLayerView[] = [];  /** Container view that vertically stacks flamechart rows */
  _verticalStackView: View;  _hoveredStackFrame: FlamechartStackFrame | null = null;
  _onHover: ((node: FlamechartStackFrame | null) => void) | null = null;  constructor(
surface: Surface,
frame: Rect,
...
const {surface, frame, _onHover, _hoveredStackFrame} = this;// Clear existing rows on data update
