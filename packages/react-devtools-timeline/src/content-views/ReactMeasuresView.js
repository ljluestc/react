...
import type {
  ...
  Rect,
  ViewRefs,
} from '../view-base';import {formatDuration} from '../utils/formatting';
...
import {
  ...
  rectIntersectsRect,
  intersectionOfRects,
} from '../view-base';import {COLORS, BORDER_SIZE, REACT_MEASURE_HEIGHT} from './constants';const REACT_LANE_HEIGHT = REACT_MEASURE_HEIGHT + BORDER_SIZE;
const MAX_ROWS_TO_SHOW_INITIALLY = 5;export class ReactMeasuresView extends View {
  _intrinsicSize: IntrinsicSize;
  _lanesToRender: ReactLane[];
  _profilerData: TimelineData;
  _hoveredMeasure: ReactMeasure | null = null;  onHover: ((measure: ReactMeasure | null) => void) | null = null;  constructor(surface: Surface, frame: Rect, profilerData: TimelineData) {
super(surface, frame);
...
const {timestamp, type, duration} = measure;let fillStyle = null;
...
let textRect = measureRect;switch (type) {
case 'commit':
...
const measuresForLane = _profilerData.laneToReactMeasureMap.get(lane);if (!measuresForLane) {
throw new Error(
...
!!_hoveredMeasure && _hoveredMeasure.batchUID === measure.batchUID;this._drawSingleReactMeasure(
context,
visibleArea,
measure,
...
const {duration, timestamp} = measure;if (
