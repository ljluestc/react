...
import {BORDER_SIZE, COLORS, NATIVE_EVENT_HEIGHT} from './constants';const ROW_WITH_BORDER_HEIGHT = NATIVE_EVENT_HEIGHT + BORDER_SIZE;export class ComponentMeasuresView extends View {
  ...
  _viewState: ViewState;  onHover: ((event: ReactComponentMeasure | null) => void) | null = null;  constructor(
surface: Surface,
frame: Rect,
profilerData: TimelineData,
viewState: ViewState,
  ) {
super(surface, frame);this._profilerData = profilerData;
this._viewState = viewState;this._cachedSearchMatches = new Map();
this._cachedSearchRegExp = null;viewState.onSearchRegExpStateChange(() => {
...
componentMeasure;const xStart = timestampToPosition(timestamp, scaleFactor, frame);
...
let typeLabel = ((null: any): string);const drawableRect = intersectionOfRects(componentMeasureRect, rect);
context.beginPath();
if (warning !== null) {
...
);const label = `${componentName} ${typeLabel} - ${formatDuration(duration)}`;drawText(label, context, componentMeasureRect, drawableRect, {
...
visibleArea,
} = this;const searchRegExp = this._viewState.searchRegExp;
...
const hoverTimestamp = positionToTimestamp(location.x, scaleFactor, frame);const componentMeasures = this._profilerData.componentMeasures;
for (let index = componentMeasures.length - 1; index >= 0; index--) {
const componentMeasure = componentMeasures[index];
const {duration, timestamp} = componentMeasure;if (
