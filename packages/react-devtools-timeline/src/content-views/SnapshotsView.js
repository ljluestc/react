...
import type {
  ...
  Surface,
  ViewRefs,
} from '../view-base';import {positioningScaleFactor, timestampToPosition} from './utils/positioning';
import {
  intersectionOfRects,
  rectContainsPoint,
  rectEqualToRect,
...
import {BORDER_SIZE, COLORS, SNAPSHOT_SCRUBBER_SIZE} from './constants';type OnHover = (node: Snapshot | null) => void;export class SnapshotsView extends View {
  _hoverLocation: Point | null = null;
  _intrinsicSize: Size;
  _profilerData: TimelineData;  onHover: OnHover | null = null;  constructor(surface: Surface, frame: Rect, profilerData: TimelineData) {
super(surface, frame);this._intrinsicSize = {
...
);const y = visibleArea.origin.y;let x = visibleArea.origin.x;// Rather than drawing each snapshot where it occurred,
...
const scaledWidth = (snapshot.width * snapshotHeight) / snapshot.height;const imageRect: Rect = {
origin: {
x,
y,
},
...
const scrubberOffset = scrubberWidth / 2;context.fillStyle = COLORS.SCRUBBER_BORDER;
context.fillRect(
...
context.drawImage(
snapshot.image,// Image coordinates
0,
0,// Native image size
snapshot.width,
snapshot.height,// Canvas coordinates
imageRect.origin.x + BORDER_SIZE,
imageRect.origin.y + BORDER_SIZE,// Scaled image size
...
frame,
);const snapshots = this._profilerData.snapshots;let startIndex = 0;
let stopIndex = snapshots.length - 1;
while (startIndex <= stopIndex) {
const currentIndex = Math.floor((startIndex + stopIndex) / 2);
const snapshot = snapshots[currentIndex];
const {timestamp} = snapshot;const snapshotX = Math.floor(
...
this._hoverLocation = null;this.setNeedsDisplay();
}onHover(null);
return;
}const snapshot = this._findClosestSnapshot(location.x);
if (snapshot !== null) {
this._hoverLocation = location;onHover(snapshot);
} else {
this._hoverLocation = null;onHover(null);
