...
import type {ViewState} from '../types';import {Surface} from './Surface';
...
import {COLORS} from '../content-views/constants';const CARET_MARGIN = 3;
const CARET_WIDTH = 5;
const CARET_HEIGHT = 3;type OnChangeCallback = (
  scrollState: ScrollState,
  containerLength: number,
) => void;export class VerticalScrollView extends View {
  _contentView: View;
  _isPanning: boolean;
  _mutableViewStateKey: string;
  _onChangeCallback: OnChangeCallback | null;
  _scrollState: ScrollState;
  _viewState: ViewState;  constructor(
surface: Surface,
frame: Rect,
contentView: View,
viewState: ViewState,
...
super(surface, frame);this._contentView = contentView;
...
this._viewState = viewState;this.addSubview(contentView);this._restoreMutableViewState();
...
const desiredSize = this._contentView.desiredSize();const above = offset;
const below = this.frame.size.height - desiredSize.height - offset;if (above < 0 || below < 0) {
const {visibleArea} = this;
const {x, y} = visibleArea.origin;
const {width, height} = visibleArea.size;
const horizontalCenter = x + width / 2;const halfWidth = CARET_WIDTH;
const left = horizontalCenter + halfWidth;
const right = horizontalCenter - halfWidth;if (above < 0) {
const topY = y + CARET_MARGIN;context.beginPath();
...
const bottomY = y + height - CARET_MARGIN;context.beginPath();
...
const desiredSize = this._contentView.desiredSize();const minimumHeight = this.frame.size.height;
const desiredHeight = desiredSize ? desiredSize.height : 0;
// Force view to take up at least all remaining vertical space.
const height = Math.max(desiredHeight, minimumHeight);const proposedFrame = {
origin: {
...
this._setScrollState(newState);return true;
...
location,
delta: {deltaX, deltaY},
} = interaction.payload;if (!rectContainsPoint(location, this.frame)) {
...
const containerHeight = this.frame.size.height;const clampedState = clampState({
state: proposedState,
...
this._scrollState.length = clampedState.length;this.setNeedsDisplay();if (this._onChangeCallback !== null) {
