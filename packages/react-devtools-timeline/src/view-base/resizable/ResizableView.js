...
import type {ViewState} from '../../types';import {ResizeBarView} from './ResizeBarView';
import {Surface} from '../Surface';
import {View} from '../View';
import {rectContainsPoint} from '../geometry';
import {noopLayout} from '../layouter';
import {clamp} from '../utils/clamp';type ResizingState = $ReadOnly<{
...
const RESIZE_BAR_WITH_LABEL_HEIGHT = 16;const HIDDEN_RECT = {
  origin: {x: 0, y: 0},
...
super(surface, frame, noopLayout);this._canvasRef = canvasRef;
...
this._viewState = viewState;this.addSubview(this._subview);
this.addSubview(this._resizeBar);this._restoreMutableViewState();
  }  desiredSize(): {+height: number, +width: number} {
const subviewDesiredSize = this._subview.desiredSize();if (this._shouldRenderResizeBar()) {
const resizeBarDesiredSize = this._resizeBar.desiredSize();return {
...
this._updateSubviewFrames();super.layoutSubviews();
...
): any): LayoutState);this._updateLayoutStateAndResizeBar(this._layoutState.barOffsetY);
} else {
...
barOffsetY = 0;
}this._layoutState.barOffsetY = barOffsetY;this._resizeBar.showLabel = barOffsetY === 0;
  }  _updateLayoutState() {
const {frame, _resizingState} = this;// Allow bar to travel to bottom of the visible area of this view but no further
const subviewDesiredSize = this._subview.desiredSize();
const maxBarOffset = subviewDesiredSize.height;let proposedBarOffsetY = this._layoutState.barOffsetY;
...
} = this;const resizeBarDesiredSize = this._resizeBar.desiredSize();if (barOffsetY === 0) {
...
this.setNeedsDisplay();return true;
}
}
