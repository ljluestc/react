...
import type {Point} from './geometry';import {useEffect, useRef} from 'react';
import {normalizeWheel} from './utils/normalizeWheel';export type ClickInteraction = {
  type: 'click',
  payload: {
event: MouseEvent,
location: Point,
...
delta: NormalizedWheelDelta,
  ...
  | WheelWithMetaInteraction;let canvasBoundingRectCache = null;
...
rect: canvas.getBoundingClientRect(),
  ...
  const didMouseMoveWhileDownRef = useRef<boolean>(false);  useEffect(() => {
...
isMouseDownRef.current = true;interactor({
type: 'mousedown',
payload: {
event,
location: localToCanvasCoordinates({x: event.x, y: event.y}),
...
isMouseDownRef.current = false;interactor({
type: 'mouseup',
payload: {
event,
location: localToCanvasCoordinates({x: event.x, y: event.y}),
...
event.stopPropagation();const location = localToCanvasCoordinates({x: event.x, y: event.y});
const delta = normalizeWheel(event);if (event.shiftKey) {
interactor({
type: 'wheel-shift',
...
ownerDocument.addEventListener('mouseup', onDocumentMouseUp);canvas.addEventListener('click', onCanvasClick);
canvas.addEventListener('dblclick', onCanvasDoubleClick);
canvas.addEventListener('mousedown', onCanvasMouseDown);
canvas.addEventListener('wheel', onCanvasWheel);return () => {
ownerDocument.removeEventListener('mousemove', onDocumentMouseMove);
ownerDocument.removeEventListener('mouseup', onDocumentMouseUp);canvas.removeEventListener('click', onCanvasClick);
