...
import type {ViewRefs} from './Surface';import {Surface} from './Surface';
import {
  rectEqualToRect,
  intersectionOfRects,
  rectIntersectsRect,
 ...
 */
export class View {
  _backgroundColor: string | null;  currentCursor: string | null = null;  surface: Surface;  frame: Rect;
  visibleArea: Rect;  superview: ?View;
  subviews: View[] = [];  /**
...
* @private
*/
  _subviewsNeedDisplay: boolean = false;  constructor(
surface: Surface,
frame: Rect,
...
context.save();
context.clip(region);
context.beginPath();this.draw(context, viewRefs);// Stop clipping
context.restore();
}
  }  /**
...
collapseLayoutIntoViews(newLayout);subviews.forEach((subview, subviewIndex) => {
...
const {subviews, visibleArea} = this;if (visibleArea.size.height === 0) {
return false;
