 ...
 */import {
  ...
  rectContainsPoint,
  unionOfRects,
} from '../geometry';describe(pointEqualToPoint, () => {
...
{origin: {x: -4, y: 2}, size: {width: 3.15, height: 4}},
  ...
  // NOTE: Undefined behavior if rects do not intersect  it('should return intersection when 2 rects intersect', () => {
// Rects touch
expect(
intersectionOfRects(
{origin: {x: 0, y: 0}, size: {width: 1, height: 1}},
...
).toEqual({origin: {x: 1, y: 1}, size: {width: 0, height: 0}});// Rects overlap
expect(
intersectionOfRects(
{origin: {x: 0, y: 0}, size: {width: 2, height: 1}},
{origin: {x: 1, y: -2}, size: {width: 0.5, height: 5}},
),
).toEqual({origin: {x: 1, y: 0}, size: {width: 0.5, height: 1}});// Rects are equal
expect(
intersectionOfRects(
