 ...
 *
 * @flow
 */import type {ReactContext} from 'shared/ReactTypes';import * as React from 'react';
import {createContext, useMemo, useReducer} from 'react';import type {ReactComponentMeasure, TimelineData, ViewState} from './types';type State = {
...
type ACTION_SET_SEARCH_TEXT = {
  ...
  | ACTION_SET_SEARCH_TEXT;type Dispatch = (action: Action) => void;const EMPTY_ARRAY: Array<ReactComponentMeasure> = [];function reducer(state: State, action: Action): State {
  let {searchIndex, searchRegExp, searchResults, searchText} = state;  switch (action.type) {
...
}const componentMeasures = state.profilerData.componentMeasures;let prevSelectedMeasureIndex = -1;for (let i = 0; i < componentMeasures.length; i++) {
const componentMeasure = componentMeasures[i];
if (componentMeasure.componentName.match(searchRegExp)) {
searchResults.push(componentMeasure);if (componentMeasure === prevSelectedMeasure) {
...
searchText,
  ...
  profilerData: TimelineData,  // Search state
  dispatch: Dispatch,
  searchIndex: number,
  searchRegExp: null,
  searchResults: Array<ReactComponentMeasure>,
...
TimelineSearchContext.displayName = 'TimelineSearchContext';type Props = {
  children: React$Node,
  profilerData: TimelineData,
  viewState: ViewState,
