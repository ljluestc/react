...
import type {TimelineData} from './types';import {importFile as importFileWorker} from './import-worker';const Pending = 0;
const Resolved = 1;
const Rejected = 2;type PendingRecord = {
  status: 0,
  value: Wakeable,
  ...
  status: 2,
  value: Error,
};type Record<T> = PendingRecord | ResolvedRecord<T> | RejectedRecord;// This is intentionally a module-level Map, rather than a React-managed one.
// Otherwise, refreshing the inspected element cache would also clear this cache.
// Profiler file contents are static anyway.
const fileNameToProfilerDataMap: Map<string, Record<TimelineData>> = new Map();function readRecord<T>(record: Record<T>): ResolvedRecord<T> | RejectedRecord {
...
throw record.value;
  ...
  let record = fileNameToProfilerDataMap.get(fileName);  if (!record) {
