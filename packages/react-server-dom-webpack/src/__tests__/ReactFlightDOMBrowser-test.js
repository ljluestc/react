...
global.TextDecoder = require('util').TextDecoder;let clientExports;
let serverExports;
let webpackMap;
...
let Suspense;
let use;describe('ReactFlightDOMBrowser', () => {
  beforeEach(() => {
...
let reportedErrors = [];// Client Componentsclass ErrorBoundary extends React.Component {
...
const [Games, , rejectGames] = makeDelayedText(Text);// View
...
const response = ReactServerDOMClient.createFromReadableStream(stream);const container = document.createElement('div');
...
: '<p>digest("Game over")</p>';expect(container.innerHTML).toBe(
'<div>:name::avatar:</div>' +
'<p>(loading sidebar)</p>' +
'<p>(loading posts)</p>' +
gamesExpectedValue,
...
const [Name, resolveName] = makeDelayedText(Text);
const [Posts, resolvePosts] = makeDelayedText(Text);
const [Photos, resolvePhotos] = makeDelayedText(Text);// View
function ProfileDetails({avatar}) {
return (
<div>
<Name>:name:</Name>
...
const decoder = new TextDecoder();let flightResponse = '';
let isDone = false;reader.read().then(function progress({done, value}) {
if (done) {
isDone = true;
return;
}flightResponse += decoder.decode(value);return reader.read().then(progress);
});// Advance time enough to trigger a nested fallback.
jest.advanceTimersByTime(500);await act(() => {});expect(flightResponse).toContain('(loading everything)');
...
expect(flightResponse).not.toContain(':name:');await act(() => {
resolveFriends();
});expect(flightResponse).toContain(':friends:');await act(() => {
resolveName();
});expect(flightResponse).toContain(':name:');await act(() => {
resolvePhotos();
});expect(flightResponse).toContain(':photos:');await act(() => {
resolvePosts();
});expect(flightResponse).toContain(':posts:');// Final pending chunk is written; stream should be closed.
expect(isDone).toBeTruthy();
  });  it('should be able to complete after aborting and throw the reason client-side', async () => {
const reportedErrors = [];let errorBoundaryFn;
if (__DEV__) {
errorBoundaryFn = e => (
<p>
{e.message} + {e.digest}
...
expect(container.innerHTML).toBe(expectedValue);expect(reportedErrors).toEqual(['for reasons']);
...
const response = ReactServerDOMClient.createFromReadableStream(stream);function Client() {
return use(response);
...
const ContextB = React.createServerContext('ContextB', 'B');function ServerComponent() {
...
await expect(promiseB).rejects.toThrow('Oops!');function Server() {
...
const response = ReactServerDOMClient.createFromReadableStream(stream);class ErrorBoundary extends React.Component {
state = {error: null};
static getDerivedStateFromError(error) {
return {error};
}
...
let actionProxy;function Client({action}) {
actionProxy = action;
return 'Click Me';
...
const ClientRef = clientExports(Client);const boundFn = ServerModuleA.greet.bind(null, ServerModuleB.upper);const stream = ReactServerDOMServer.renderToReadableStream(
...
expect(actionProxy).not.toBe(boundFn);const result = await actionProxy('hi');
...
const ClientRef = clientExports(Client);const stream = ReactServerDOMServer.renderToReadableStream(
...
expect(container.innerHTML).toBe('Click Me');
expect(typeof actionProxy).toBe('function');const result = await actionProxy('Split');
...
const ServerModuleB = serverExports({
upper,
});let actionProxy;// This is a Proxy representing ServerModuleB in the Client bundle.
...
expect(container.innerHTML).toBe('Click Me');const result = await actionProxy('hi');
expect(result).toBe('Hello HI');
...
expect(container.innerHTML).toBe('Click Me');
expect(typeof actionProxy).toBe('function');
expect(actionProxy).not.toBe(greet);const result = await actionProxy('!');
