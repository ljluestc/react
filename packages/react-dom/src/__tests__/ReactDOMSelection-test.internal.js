...
let ReactDOM;
let ReactDOMSelection;let getModernOffsetsFromPoints;describe('ReactDOMSelection', () => {
  beforeEach(() => {
React = require('react');
ReactDOM = require('react-dom');
ReactDOMSelection = require('react-dom-bindings/src/client/ReactDOMSelection');({getModernOffsetsFromPoints} = ReactDOMSelection);
...
let start;
let end;
let length = 0;function traverse(node) {
...
traverse(outerNode);if (start === null || end === null) {
...
expect(allNodes.length).toBe(41);function randomNode() {
...
const focusOffset = randomOffset(focusNode);const offsets1 = getModernOffsetsFromPoints(
fixtureRoot,
anchorNode,
anchorOffset,
focusNode,
