...
let act;
let container;
let assertLog;jest.useRealTimers();global.IS_REACT_ACT_ENVIRONMENT = true;function sleep(period) {
...
rerenderConcurrent,
  ...
  }  // and then in legacy mode  let legacyDom = null;
...
ReactDOM.render(el, legacyDom);
  }  runActTests('legacy mode', renderLegacy, unmountLegacy, rerenderLegacy);  describe('unacted effects', () => {
function App() {
React.useEffect(() => {}, []);
return null;
...
act = ReactTestUtils.act;const InternalTestUtils = require('internal-test-utils');
...
jest.runAllTimers();
});expect(container.innerHTML).toBe('1');act(() => {
jest.runAllTimers();
...
spyOnDevAndProd(console, 'error').mockImplementation(() => {});await Promise.all([
act(async () => {
await sleep(50);
}),
act(async () => {
...
// exiting act() drains effects and microtasksassertLog([0, 1]);
expect(container.innerHTML).toBe('1');
});// @gate __DEV__
...
// todo - remove __DEV__ check once we start using testing builds// @gate __DEV__
...
).not.toBeNull();// now render regular content again
act(() => {
rerender(<App suspend={false} />);
});
expect(document.querySelector('[data-test-id=spinner]')).toBeNull();// trigger a suspendy update with a delay
