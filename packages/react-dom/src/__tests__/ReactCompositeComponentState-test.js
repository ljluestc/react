...
let ReactDOM;let TestComponent;describe('ReactCompositeComponent-state', () => {
  beforeEach(() => {
React = require('react');
ReactDOM = require('react-dom');TestComponent = class extends React.Component {
constructor(props) {
super(props);
...
document.body.appendChild(container);const stateListener = jest.fn();
...
instance.forceUpdate(instance.peekAtCallback('forceUpdate'));ReactDOM.unmountComponentAtNode(container);const expected = [
...
const container = document.createElement('div');let ops = [];let child = null;
let parent = null;class Child extends React.Component {
state = {bar: false};
componentDidMount() {
child = this;
}
...
}let shouldUpdate = true;class Intermediate extends React.Component {
...
}ReactDOM.render(<Parent />, container);ReactDOM.unstable_batchedUpdates(() => {
...
expect(ops).toEqual(['child did update', 'parent did update']);shouldUpdate = false;ops = [];ReactDOM.unstable_batchedUpdates(() => {
...
expect(el.textContent).toBe('count:1');ReactDOM.render(<Child incrementBy={2} />, el);
expect(el.textContent).toBe('count:3');ReactDOM.render(<Child incrementBy={1} />, el);
...
expect(el.textContent).toBe('A');ReactDOM.render(<B />, el);
