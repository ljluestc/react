...
let textCache;
let assertLog;describe('ReactDOMFizzShellHydration', () => {
  beforeEach(() => {
jest.resetModules();
...
Stream = require('stream');const InternalTestUtils = require('internal-test-utils');
assertLog = InternalTestUtils.assertLog;startTransition = React.startTransition;textCache = new Map();// Test Environment
...
container = document.getElementById('container');buffer = '';
...
Scheduler.log(`Suspend! [${text}]`);const thenable = {
pings: [],
then(resolve) {
...
const div = React.createRef(null);function App() {
return (
<div ref={div}>
...
assertLog(['Shell']);
const dehydratedDiv = container.getElementsByTagName('div')[0];// Clear the cache and start rendering on the client
resetTextCache();// Hydration suspends because the data for the shell hasn't loaded yet
await clientAct(async () => {
ReactDOMClient.hydrateRoot(container, <App />);
});
assertLog(['Suspend! [Shell]']);
expect(div.current).toBe(null);
expect(container.textContent).toBe('Shell');// The shell loads and hydration finishes
await clientAct(async () => {
await resolveText('Shell');
});
assertLog(['Shell']);
...
assertLog(['Suspend! [Shell]']);await clientAct(async () => {
await resolveText('Shell');
});
assertLog(['Shell']);
expect(container.textContent).toBe('Shell');
...
assertLog(['Initial']);await clientAct(async () => {
...
assertLog(['Shell']);// Clear the cache and start rendering on the client
resetTextCache();// Hydration suspends because the data for the shell hasn't loaded yet
...
assertLog(['Suspend! [Shell]']);
expect(container.textContent).toBe('Shell');await clientAct(async () => {
...
spyOnDevAndProd(console, 'error').mockImplementation(() => {});function NestedComponent({depth}: {depth: number}) {
if (depth <= 0) {
return <AsyncText text="Shell" />;
}
