 ...
 */let ReactDOMS  jest.resetModules();  ReactDOM = require('rea  ReactTestUtils = require('react-dom/test-utils');  // Make them available to the helpers.
  return {
ReactDOM,
ReactDOMServer,  };
}const {
  ...
  serverRender,  clientCleanRender,
...
expect(e.childNodes.length).toBe(1);});itRenders('a div with text with flanking whitespace', async render => {
// prettier-ignore
const e = await render(<div>  Text </div>);
expec});itRenders('a div with an empty text child', async render => {
...
expexpect(spanNode.childNodes.length).toBe(1);
...
}).toErrorDev('The tag <nonstandard> is unrecognized in this browser.');const e = await render(<nonstandard>Text</nonstandard>);
...
expect(e.childNodes.length).toBe(2);expectTextNode(e.childNodes[1], 'bar');
}
});itRenders(
'a component returning text node between two text nodes',
asyconst e = await render(
<div>
{'a'}
<B />
{'c'}
...
} eexpectTextNode(e.childNodes[0], 'a');
...
irender === clientRenderOnServerString ||
render === streamRender
)expect(e.childNodes.length).toBe(3);
expectTextNode(e.childNodes[0], 'foo');
...
const eexpectTextNode(e.childNodes[0], 'foo');
...
let<image xlinkhref="http://i.imgur.com/w7GCRPb.png" />
</svg>,
1,
);
e = e.firstChild;
...
expect(const e = await render(<button />);
expect(e.childNodes.length).toBe(0);
expect(e.nextSibling).toBe(null);
...
).fexpect(e.firstChild.nodeType).toBe(TEXT_NODE_TYPE);
...
async render => {
const e = await render(
<diexpect(e.childNodes.length).toBe(1);
...
async render => {
const e = await render(
<diexpect(e.childNodes.length).toBe(0);
},
);itRenders(
...
for (leexpect(e.childNodes.length).toBe(1);
e = e.firstChild;
}
expect(e.tagName).toBe('DIV');
expect(e.childNodes.length).toBe(0);
...
async render => {
const e<div id="child1" /> <div id="child2" />
</div>,
);
expect(e.id).toBe('parent');
econst textNode = e.childNodes[1];
const child2 = e.childNodes[2];
expect(child1.id).toBe('child1');
expexpect(child2.id).toBe('child2');
...
async render => {
// prettier-ignore
conconst textNode1 = e.childNodes[0];
...
expectTexpectTextNode(parent.childNodes[2], 'c');
}
});
...
// We won't be patching up in this case as that matches our past behavior.itRenders(
...
ex// Client rendering uses JS value with CR and null character.
...
awconst UndefinedComponent = () => undefined;
...
'Ob? ' If you meant to render a collection of children, use ' +
'an array instead.'
: ''),
);itThrowsWhenRendering(
'top-level object',
...
}).toEr'(for built-in components) or a class/function (for composite ' +
