...
let Scheduler;
let act;
let textCache;describe('ReactDOMSuspensePlaceholder', () => {
...
expect(window.getComputedStyle(divs[2].current).display).toEqual('none');await act(async () => {
await resolveText('B');
...
ReactDOM.render(<App />, container);
expect(container.textContent).toEqual('Loading...');await act(async () => {
await resolveText('B');
...
async () => {
const {useState} = React;let setIsVisible;
...
}ReactDOM.render(<App />, container);expect(log).toEqual(['cDM first', 'cDM second']);
log.length = 0;buttonRef.current.dispatchEvent(new MouseEvent('click', {bubbles: true}));
await Lazy;
