...
let originalHTMLImageElementSrcDescriptor;let images = [];
let onLoadSpy = null;
let actualLoadSpy = null;let waitForAll;
let waitFor;
let assertLog;function PhaseMarkers({children}) {
...
assertLog = InternalTestUtils.assertLog;onLoadSpy = jest.fn(reactEvent => {
...
//([text, ms]) => text,
// );
// textResourceShouldFail = false;images = [];originalCreateElement = document.createElement;
...
const root = ReactDOMClient.createRoot(container);React.startTransition(() =>
root.render(
<PhaseMarkers>
...
const container = document.createElement('div');
const root = ReactDOMClient.createRoot(container);function Base() {
...
return null;
}React.startTransition(() => root.render(<Base />));await waitFor(['render start', 'Img a', 'Yield']);
const img = last(images);
loadImage(img);
assertLog(['actualLoadSpy [a]']);await waitFor([
'load triggered',
'last layout',
...
const container = document.createElement('div');
const root = ReactDOMClient.createRoot(container);function ChildSuppressing({children}) {
...
const container = document.createElement('div');
const root = ReactDOMClient.createRoot(container);function Switch({children}) {
...
expect(onLoadSpy).not.toHaveBeenCalled();
  ...
  //);
  //}  //const container = document.createElement('div');
  //const root = ReactDOMClient.createRoot(container);  //React.startTransition(() => root.render(<SuspendingWithImage />));  //expect(Scheduler).toFlushAndYield([
  //'SuspendingWithImage',
  //'Suspend! [A]',
  //'render start',
  //'Img default',
  ...
  //expect(onLoadSpy).not.toHaveBeenCalled();  //// Flush some of the time
  //jest.advanceTimersByTime(50);
  //// Still nothing...
  //expect(Scheduler).toFlushWithoutYielding();  //// Flush the promise completely
  //jest.advanceTimersByTime(50);
  //// Renders successfully
  //expect(Scheduler).toHaveYielded(['Promise resolved [A]']);  //expect(Scheduler).toFlushAndYieldThrough([
  //'A',
  ...
  //'actualLoadSpy [default]',
  //'onLoadSpy [default]',
  //]);  //expect(Scheduler).toFlushAndYield(['last passive']);  //expect(onLoadSpy).toHaveBeenCalledTimes(1);
...
let externalSetSrcAlt = null;function Base() {
...
const root = ReactDOMClient.createRoot(container);root.render(<Base />);await waitForAll([]);React.startTransition(() => externalSetSrc('a'));await waitFor(['YieldingWithImage', 'Img a', 'Yield']);
let img = last(images);
loadImage(img);
assertLog(['actualLoadSpy [a]']);ReactDOM.flushSync(() => externalSetSrcAlt('b'));assertLog([
'YieldingWithImage',
'Img b',
'Yield',
'b',
...
img = last(images);
expect(img.__needsDispatch).toBe(true);
loadImage(img);assertLog(['actualLoadSpy [b]', 'onLoadSpy [b]']);
...
// where the resetting of the src property caused the property to change from relative to fully qualified// make sure we are not using the patched src setter
Object.defineProperty(
HTMLImageElement.prototype,
'src',
originalHTMLImageElementSrcDescriptor,
...
const renderSrcAttr = img.getAttribute('src');// finish render and commit causing the src property to be rewritten
await waitForAll(['a', 'last layout', 'last passive']);
const commitSrcProperty = img.src;
const commitSrcAttr = img.getAttribute('src');// ensure attribute and properties agree
