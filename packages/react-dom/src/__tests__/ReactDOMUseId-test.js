...
let waitForPaint;describe('useId', () => {
  beforeEach(() => {
jest.resetModules();
...
useId = React.useId;
useState = React.useState;const InternalTestUtils = require('internal-test-utils');
waitForPaint = InternalTestUtils.waitForPaint;// Test Environment
...
const {StrictMode} = React;function App() {
return (
<StrictMode>
<DivWithId />
</StrictMode>
...
// of any size.function Child({children}) {
const id = useId();
return <div id={id}>{children}</div>;
}function App() {
let tree = <Child />;
...
const divs = container.querySelectorAll('div');// Confirm that every id matches the expected pattern
...
expect(span.current).toBe(null);// Swap B for C
