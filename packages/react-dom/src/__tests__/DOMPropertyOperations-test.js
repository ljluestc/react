 ...
 *
 * @emails react-core
 */  enableCusto} = require('shared/ReactFeatureFlags');describe('DOMPropertyOperations', () => {
  let React;
  let ReactDOM;  beforeEach(() => {
jest.resetModules();
React = require('react');  });  // Sets a value in a way that React doesn't see,
...
const container = document.createElement('div');
ReactDOReactDOM.render(<div disabled={true} />, container);
...
const cexpect(container.firstChild.hasAttribute('allowFullScreen')).toBe(false);
...
const container = document.createElement('div');
ReactDOReactDOM.render(<div className={null} />, container);
...
const cspyOnDevAndProd(container.firstChild, 'setAttribute');
...
const container = document.createElement('div');
ReactDOM.render(<Test />, container);
contain.dispatchEvent(new Event('Customevent'));
expect(oncustomevent).toHaveBeenCalledTimes(1);
});// @gate enableCustomElementPropertySupport
...
const oreturn <my-custom-element oncustomevent={props.handler} />;
}const container = document.createElement('div');
ReactDOM.render(<Test handler={oncustomevent} />, container);
const customElement = container.querySelector('my-custom-element');
customElement.dispatchEvent(new Event('customevent'));
expect(oncustomevent).toHaveBeenCalledTimes(1);ReactDOM.render(<Test handler={false} />, container);
...
customElement.dispatchEvent(new Event('customevent'));
expect(oncustomevent).toHaveBeenCalledTimes(1);ReactDOM.render(<Test handler={oncustomevent} />, container);
customElement.dispatchEvent(new Event('customevent'));
expect(customElement.dispatchEvent(new Event('customevent'));
...
customEcustomElement.dispatchEvent(new Event('true'));
...
elet nativeClickEvent = null;
const nativeEventHandler = jest.fn(event => (nativeClickEvent = event));
function Test() {
return <my-custom-element onClick={syntheticEventHandler} />;
}const container = document.createElement('div'ReactDOM.render(<Test />, container);const customElement = container.querySelector('my-custom-element');
customElement.onclick = nativeEventHandler;
container.querySelector('my-custom-element').click();expect(nativeEventHandler).toHaveBeenCalledTimes(1);
expect(syntheticEventHandler).toHaveBeenCalledTimes(1);
expect(syntheticClickEvent.nativeEvent).toBe(nativeClickEvent);
});// @gate enableCustomElementPropertylet reactChangeEvent = null;
const eventHandler = jest.fn(event => (reactChangeEvent = event));
const container = document.createElement('div')ReactDOM.render(<my-custom-element onChange={eventHandler} />, container);
const customElement = container.querySelector('my-custom-element');
let expectedHandlerCallCount = 0;const changeEvent = new Event('change', {bubbles: true});
customElement.dispatchEvent(changeEvent);
expectedHandlerCallCount++;
expect(ReactDOM.render(<my-custom-element />, container);
...
const customElement = container.querySelector('my-custom-element');
let expectedHandlerCallCount = 0;const inputEvent = new Event('input', {bubbles: true});
customElement.dispatchEvent(inputEvent);
expectedHandlerCallCount++;
expect(eventHandler).toHaveBeenCalledTimes(expectedHandlerCallCount);
expect(reactInputEvent.nativeEvent).toBe(inputEvent);// Also make sure that removing and re-adding the event listener works
ReactDOM.render(<my-custom-element />, container);
customElement.dispatchEvent(new Event('input', {bubbles: true}));
excustomElement.dispatchEvent(new Event('input', {bubbles: true}));
expectedHandlerCallCount++;
expect(eventHandler).toHaveBeenCalledTimes(expectedHandlerCallCount);
});// @gate enableCustomElementPropertySupport
it('custom elements should have separate onInput and onChange handling', () => {
const container = document.createElement('div');
documenconst changeEventHandler = jest.fn();
ReactDOM.render(
<my-custom-element
...
const customElement = container.querySelector('my-custom-element');customElement.dispatchEvent(new Event('input', {bubbles: true}));
eexpect(inputEventHandler).toHaveBeenCalledTimes(1);
expect(changeEventHandler).toHaveBeenCalledTimes(1);
});// @gate enableCustomElemeconst container = document.createElement('div');
document.body.appendChild(container);
const eventHandler = jest.fn();<my-custom-element oncustomevent={eventHandler} />,
container,
);const customElement = container.querySelector('my-custom-element');
customEcustomElement.dispatchEvent(new Event('customevent'));
expect(eventHandler).toHaveBeenCalledTimes(1);ReactDOM.render(
...
const container = document.createconst regularOnInputHandler = jest.fn();
const regularOnChangeHandler = jest.fn();
const regularOnClickHandler = jest.fn();
const customOnInputHandler = jest.fn();
const customOnChangeHandler = jest.fn();function clearMocks() {
...
onInput={customOnInputHandler}onClick={customOnClickHandler}
/>
</div>,
container,
...
expect(regularInput).not.toBe(customInput);// Typing should trigger onInput and onChange for both kinds of inputs.
clearMocks();
setUntrackedValue.call(regularInput, 'hello');
regularInput.dispatchEvent(new Event('input', {bubbles: true}));
expect(expect(regularOnClickHandler).toHaveBeenCalledTimes(0);
...
expect(customOnClickHandler).toHaveBeenCalledTimes(0);// The native change event itself does not produce extra React events.
clearMocks();
...
expect(customOnChangeHandler).toHaveBeenCalledTimes(0);clearMocks();
regularInput.dispatchEvent(new Event('click', {bubbles: true}));
expect(regularOnInputHandler).toHaveBeenCalledTimes(expect(regularOnClickHandler).toHaveBeenCalledTimes(1);
...
customInput.dispatchEvent(new Event('input', {bubbles: true}));
exexpect(customOnClickHandler).toHaveBeenCalledTimes(0);
});it('<input type=radio is=...> should have the same onChange/onInput/onClick behavior as <input type=radio>', () => {
const container = document.createElement('div'const regularOnInputHandler = jest.fn();
const regularOnChangeHandler = jest.fn();
const regularOnClickHandler = jest.fn();const customOnChangeHandler = jest.fn();
const customOnClickHandler = jest.fn();
function clearMocks() {
regularOnInputHandler.mockClear();
regularOnChangeHandler.mockClear();
regularOnClickHandler.mockClear();
customOcustomOnClickHandler.mockClear();
}
ReactDOM.render(
<div>
<input
...
expect(regularInput).not.toBe(customInput);// Clicking should trigger onClick and onChange on both inputs.
clearMocks();
...
seexpect(customOnInputHandler).toHaveBeenCalledTimes(0);
expect(customOnChangeHandler).toHaveBeenCalledTimes(1);
expect(customOnClickHandler).toHaveBeenCalledTimes(1);// The native input event only produces a React onInput event.
clearMocks();
regularInput.dispatchEvent(new Event('input', {bubbles: true}));
expect(regularOnInputHandler).toHaveBeenCalledTimes(1);
expect(regularOnChangeHandler).toHaveBeenCacustomInput.dispatchEvent(new Event('input', {bubbles: true}));
expect(customOnInputHandler).toHaveBeenCalledTimes(1);
expect(customOnChangeHandler).toHaveBeenCalledTimes(0);
expect(customOnClickHandler).toHaveBeenCalledTimes(0);// Clicking again should trigger onClick and onChange on both inputs.
clearMocks();
...
customInput.dispatchEvent(new Event('click', {bubbles:expect(customOnChangeHandler).toHaveBeenCalledTimes(1);
...
ReactDOM.render(
<div>onInput={regularOnInputHandler}
onChange={regularOnChangeHandler}
onClick={regularOnClickHandler}
/>
<select
...
expect(regularSelect).not.toBe(customSelect);// Clicking should only trigger onClick on both inputs.
clearMocks();
...
expect(customOnInputHandler).toHaveBeenCalledTimes(0);
expect(customOnChangeHandler).toHaveBeenCalledTimes(0);
expect(customOnClickHandler).toHaveBeenCalledTimes(1);// Native input event should only trigger onInput on both inputs.
clearMocks();
...
expect(customOnInputHandler).toHaveBeenCalledTimes(1);
expect(customOnChangeHandler).toHaveBeenCalledTimes(0);
expect(customOnClickHandler).toHaveBeenCalledTimes(0);// Native change event should trigger onChange.
clearMocks();
...
const customElement = container.querySelector('my-cust'input:not([is="my-custom-element"])',
);
const customInput = container.querySelector(
'input[is="my-custom-element"]',
);
expect(regularInput).not.toBe(customInput);// Custom element has no special logic for input/change.
clearMocks();
customElement.dispatchEvent(new Event('input', {bubbles: true}));
expect(onChangeHandler).toBeCalledTimes(0);
expect(onInputHandler).toBeCalledTimes(1);customElement.dispatchEvent(new Event('change', {bubbles: true}));
...
expect(onChangeHandler).toBeCalledTimes(1);
expect(onInputHandler).toBeCalledTimes(1);
expect(onClickHandler).toBeCalledTimes(1);// Regular input treats browser input as onChange.
clearMocks();
setUntrackedValue.call(regularInput, 'hello');
regularInput.dispatchEvent(new Event('input', {bubbles: true}));
expect(onChangeHandler).toBeCalledTimes(1);
expect(regularInput.dispatchEvent(new Event('change', {bubbles: true}));
expect(onChangeHandler).toBeCalledTimes(1);
expect(onInputHandler).toBeCalledTimes(1);
expect(onClickHandler).toBeCalledTimes(0);
regularInput.dispatchEvent(new Event('click', {bubbles: true}));
expect(onChangeHandler).toBeCalledTimes(1);
expect(onInputHandler).toBeCalledTimes(1);
expect(onClickHandler).toBeCalledTimes(1);// Custom input treats browser input as onChange.
clearMocks();
setUntrackedValue.call(customInput, 'hello');
customInput.dispatchEvent(new Event('input', {bubbles: true}));
expect(onChangeHandler).toBeCalledTimes(1);
...
// bubbles to the parent custom element.expect(onInputHandler).toBeCalledTimes(1);
expect(onClickHandler).toBeCalledTimes(0);
...
expect(onChangeHandler).toBeCalledTimes(1);
expect(onInputHandler).toBeCalledTimes(1);});it('custom element onChange/onInput/onClick with event target div child', () => {
const container = document.createElement('div');
document.body.appendChild(container);
const onChangeHandler = jest.fn();
const onInputHandler = jest.fn();
...
expect(onChangeHandler).toBeCalledTimes(0);
expect(onInputHandler).toBeCalledTimes(1);
expect(onClickHandler).toBeCalledTimes(0);div.dispatchEvent(new Event('change', {bubbles: true}));
...
expect(expect(onClickHandler).toBeCalledTimes(0);div.dispatchEvent(new Event('click', {bubbles: true}));
expect(onChangeHandler).toBeCalledTimes(0);
expect(onInputHandler).toBeCalledTimes(1);
expect(onClickHandler).toBeCalledTimes(1);
...
expect(onChangeHandler).toBeCalledTimes(0);
expect(onInputHandler).toBeCalledTimes(1);
expect(onClickHandler).toBeCalledTimes(0);div.dispatchEvent(new Event('click', {bubbles: true}));
expect(onChangeHandler).toBeCalledTimes(0);
expect(onInputHandler).toBeCalledTimes(1);
expect(onClickHandler).toBeCalledTimes(1);it('custom element onChange/onInput/onClick with event target custom element child', () => {
const container = document.createElement('div');
document.body.appendChild(container);
const onChangeHandler = jest.fn();
const onInputHandler = jest.fn();
...
container,customChild.dispatchEvent(new Event('input', {bubbles: true}));
...
expect(onInputHandler).toBeCalledTimes(1);
expect(onClickHandler).toBeCalledTimes(1);
});// @gate enableCustomElementProconst oncustomeventCapture = jest.fn();
const oncustomevent = jest.fn();
function Test() {
return (
<my-custom-element
...
.dispatexpect(oncustomevent).toHaveBeenCalledTimes(0);
...
expect(customElement.hasChildNodes()).toBe(false);// Render again to verify the update codepath doesn't accidentally let
// something through.
...
const container = document.createElement('div');
Reexpect(customElement.getAttribute('innerText')).toBe(null);
expect(customElement.hasChildNodes()).toBe(false);// Render again to verify the update codepath doesn't accidentally let
// something through.
...
ReactDOM.render(<my-custom-element />, container);
const customElement = container.querySelector('my-custom-element');
customElement.foo = null;// true => string
...
expect(customElement.foo).toBe('bar');// false => string
ReactDOM.render(<my-custom-element foo={false} />, container);
expect(customElement.foo).toBe(false);
ReactDOM.render(<my-custom-element foo="bar" />, container);
expect(customElement.foo).toBe('bar');// true => null
ReactDOM.render(<my-custom-element foo={true} />, container);
exexpect(customElement.foo).toBe(null);// false => null
ReactDOM.render(<my-custom-element foo={false} />, container);
expect(customElement.foo).toBe(false);
ReactDOM.render(<my-custom-element foo={null} />, container);
expect(customElement.foo).toBe(null);it('boolean props should not be stringified in attributes', () => {
const container = document.createElement('div');
document.body.appendChild(container);
ReactDOM.render(<my-custom-element foo={true} />, container);
const customElement = container.querySelector('my-custom-element');expect(customElement.getAttribute('foo')).toBe('');// true => false
ReactDOM.render(<my-custom-element foo={fait('custom element custom event handlers assign multiple types', () => {
const container = document.createElement('div');
document.body.appendChild(container);
const oncustomevent = jest.fn();// First render with string
ReactDOcustomelement.dispatchEvent(new Event('customevent'));
expect(oncustomevent).toHaveBeenCalledTimes(0);
expect(customelement.oncustomevent).toBe(undefined);
expect(customelement.getAttribute('oncustomevent')).toBe('foo');// string => event listener
ReactDOM.render(
...
expect(oncustomevent).toHaveBeenCalledTimes(1);
expect(customelement.oncustomevent).toBe(undefined);
expect(customelement.getAttribute('oncustomevent')).toBe(null);// event listener => string
ReactDOM.render(<my-custom-element oncustomevent={'foo'} />, container);
customelement.dispatchEvent(new Event('customevent'));
exexpect(customelement.getAttribute('oncustomevent')).toBe('foo');// string => nothing
ReactDOM.render(<my-custom-element />, container);
customelement.dispatchEvent(new Event('customevent'));
expect(oncustomevent).toHaveBeenCalledTimes(1);
expect(customelement.oncustomevent).toBe(uReactDOM.render(
<my-custom-element oncustomevent={oncustomevent} />,
container,
);
customelement.dispatchEvent(new Event('customevent'));
expect(oncustomevent).toHaveBeenCalledTimeexpect(customelement.getAttribute('oncustomevent')).toBe(null);
});// @gate enableCustomElementPropertySupport
it('custom element custom event handlers assign multiple types with setter', () => {
const container = document.createElement('div');
documenReactDOM.render(<my-custom-element />, container);
const customelement = container.querySelector('my-custom-element');
...
expect(customelement.oncustomevent).toBe(undefined);// nothing => event listener
ReactDOM.render(
<my-custom-element oncustomevent={oncustomevent} />,
container,
);expect(oncustomevent).toHaveBeenCalledTimes(1);
...
expect(customelement.getAttribute('oncustomevent')).toBe(null);// string => event listener
ReactDOM.render(
<my-custom-element oncustomevent={oncustomevent} />,
container,
);
...
const customElement = container.querySelector('my-custom-element');
expect(customElement.getAttribute('foo')).toBe('one');// Install a setter to activate the `in` heuristic
...
expect(});// @gate enableCustomElementPropertySupport
it('custom element properties should accept functions', () => {
const container = document.createElement('div');
document.body.appendChild(container);
ReactDOM.render(<my-custom-element />, container);
const customElement = container.querySelector('my-set: function (x) {
this._foo = x;
},
get: function () {
return this._foo;
...
expect(customElement.foo).toBe(myFunction);// Also remove and re-add the property for good measure
ReactDOM.render(<my-custom-element />, container);
expect(customElement.foo).toBe(null);expect(customElement.foo).toBe(myFunction);
});
  });  describe('deleteValueForProperty', () => {
it('should remove attributes for normal properties', () => {
const container = document.createElement('div');
ReactDOReactDOM.render(<div />, container);
expect(container.firstChild.getAttribute('title')).toBe(null);
});it('should not remove attributes for special properties', () => {
const container = document.createElement('div');
ReactDOM.render(
<input type="text" value="foo" onChange={function () {}} />,
container,if (disableInputAttributeSyncing) {
expect(container.firstChild.hasAttribute('value')).toBe(false);
} else {
...
expect(container.firstChild.value).toBconst container = document.createElement('div');
