 ...
 *
 * @emails react-core
 */let ReactDOMS  beforeEach(() => {
jest.resetModules();React = require('react');
ReactDOM = require('react-dom');
ReactDOMServer = requiReactFeatureFlags.disableLegacyContext = true;
...
returreturn JSON.stringify(val);
...
LegacyFnConsumer.contextTypes = {foo() {}};function RegularFn(props, context) {
return formatValue(context);
...
'Use React.createContext() instead.','Use React.createContext() with static contextType instead.',
...
expect(lifecycleContextLog).toEqual([]);// Test update path.
ReactDOM.render(
<LegacyProvider>
<span>
<LegacyClsConsumer />
...
ReactDOM.unmountComponentAtNode(container);// test server path.
let text;
expect(() => {
...
expect(lifecycleContextLog).toEqual([{}, {}const Ctx = React.createContext();class Provider extends React.Component {
render() {
return (
...
lifecrender() {
return formatValue(this.context);}function FnConsumer() {
...
lifecycleContextLog.length = 0;ReactDOM.render(
<Provider value="b">
<span>
<RenderPropConsumer />
<ContextTypeConsumer />
