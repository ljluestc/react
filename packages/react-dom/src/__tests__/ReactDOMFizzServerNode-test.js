 ...
 */'use strict';let Stream;
let React;
let ReactDOMFizzServer;
let Suspense;describe('ReactDOMFizzServerNode', () => {
  beforeEach(() => {
jest.resetModules();
React = require('react');
...
// Resolve the loading.
hasLoaded = true;
await resolve();await jest.runAllTimers();expect(output.result).toBe('');
expect(isCompleteCalls).toBe(1);// First we write our header.
output.result +=
'<!doctype html><html><head><title>test</title><head><body>';
// Then React starts writing.
pipe(writable);
...
pipe(writable);await completed;expect(output.error).toBe(theError);
expect(output.result).toBe('');
...
pipe(writable);await completed;expect(output.error).toBe(undefined);
...
const {writable, output, completed} = getTestWritable();let renderedFallback = false;
function Fallback() {
...
pipe(writable);await completed;expect(output.result).toContain('Hi');
...
pipe(writable);jest.runAllTimers();expect(output.result).toContain('Loading');
expect(isCompleteCalls).toBe(0);abort(new Error('uh oh'));await completed;expect(errors).toEqual(['uh oh']);
...
pipe(writable);// Currently we delay work so if we abort, we abort the remaining CPU
// work as well.// Abort before running the timers that perform the work
const theReason = new Error('uh oh');
abort(theReason);jest.runAllTimers();await completed;expect(errors).toEqual(['uh oh']);
...
pipe(writable);jest.runAllTimers();expect(output.result).toContain('Loading');
expect(isCompleteCalls).toBe(0);abort();await completed;expect(errors).toEqual([
...
).pipe(writable);jest.runAllTimers();expect(output.error).toBe(undefined);
expect(output.result).toContain('loading');await completed;expect(output.error).toBe(undefined);
...
).pipe(writable0);const client1 = new DelayClient();
const {
writable: writable1,
output: output1,
...
).pipe(writable1);jest.runAllTimers();expect(output0.error).toBe(undefined);
expect(output0.result).toContain('loading');expect(output1.error).toBe(undefined);
expect(output1.result).toContain('loading');await Promise.all([completed0, completed1]);expect(output0.error).toBe(undefined);
expect(output0.result).not.toContain('context never found');
expect(output0.result).toContain('OK');expect(output1.error).toBe(undefined);
...
pipe(writable);expect(rendered).toBe(false);
expect(isComplete).toBe(false);writable.end();await jest.runAllTimers();hasLoaded = true;
resolve();await completed;expect(errors).toEqual([
'The destination stream errored while writing data.',
]);
