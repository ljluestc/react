 ...
 */  let OuterRe  let container;  beforeEach(() => {
...
conta  }  function render(tree) {
cleanup();
contaOuterReactDOM.render(tree, container);
...
disnew Event('lostpointercapture', {
bubbles: true,
cancelable: true,
}),
);
...
testNativeBubblingEvent({
typreactEventType: 'pointercancel',
...
parlog.push('--- inner parent enter');
},
onPointerLeave: e => {
log.push('--- inner parent leave');
},
...
outlog.push('- outer parent enter');
},
onPointerLeave: e => {
log.push('- outer parent leave');
},
...
'value',
  ).set;  // The tests for these events are currently very limited
  // because they are fully synthetic, and so they don't
  // work very well across different roots. For now, we'll
  // just document the current state in these tests.
...
outerParentProps={{
[eventConfig.reactEvent]: e => {
log[eventConfig.reactEvent + 'Capture']: e => {
expect(e.type).toBe(eventConfig.reactEventType);
log.push('- outer parent capture');
},
}}
