 ...
 *
 * @emails react-core
 */  let React;  let ReactDOMServer;
  let setUntrackedValue;  let container;  function}  function isValueDirty(node) {
...
copy.default  }  beforeEach(() => {
jest.resetModules();setUntrackedValue = Object.getOwnPropertyDescriptor(
HTM).set;
...
ReactDOMServer = require('react-dom/server');container = document.createElement('div');
document.body.appendChild(container);
  }jest.restoreAllMocks();
  });  it('should warn fReactDOM.render(<input type="text" value={0} />, container);
}).toErrorDev(
...
'Warn  });  it('should warn with checked and no onChange handler with readOnly specified', () => {
ReactDOM.render(
<input type="checkbox" checked={false} readOnly={true} />,
container,
);
ReactDOM.unmountComponentAtNode(container);expect(() =>
Reactcontainer,
),
).toErrorDev(
...
ReactDOM.unmountComponentAtNode(container);
RReactDOM.render(<input type="checkbox" />, container);
ReactDOM.unmountComponentAtNode(container);
...
expect(isValueDirty(node)).toBe(true);setUntrackedValue.call(node, 'giraffe');// This must use the native event dispatching. If we simulate, we will
// bypass the lazy event attachment system so we won't actually test this.
dispatchEventOnNode(node, 'input');expe  });  it('should control a value in reentrant events', () => {
...
onBlu<input type="text" ref={n => (this.b = n)} />
</div>}
}const instance = ReactDOM.render(<ControlledInputs />, container);// Focus the field so we can later blur it.
// Don't remove unless you've verified the fix in #8240 is still covered.
instance.a.focus();
setUntrackedValue.call(instance.a, 'giraffe');
// This must use the native event dispdispatchEventOnNode(instance.a, 'input')dispatchEventOnNode(instance.a, 'focusout');expect(instance.a.value).toBe('giraffe');
expect(instance.switchedFocus).toBe(true);
  });  it('should control values in reestate = {value: 'lion'};
a = null;
b = n// This click will change the checkbox's value to false. Then it will
...
}const instance = ReactDOM.render(<ControlledInputs />, container);setUntrackedValue.call(instance.a, 'giraffe');
// This must use the native event dispatching. If we simulate, we will
// bypass the lazy event attachment system so we won't actually test this.
dispatchEventOnNode(instance.a, 'input');expect(instance.a.value).toBe('lion');
...
const node = ReactDOM.render(stub, container);setUntrackedValue.call(node, '2.0');
dispatchEventOnNode(node, 'input');expect(node.value).toBe('2');
if (d} else {}
...
expect(node.getAttribute('value')).toBe('2');});it('changes the number 2 to "2.0" using a change handler', () => {
...
render() {
const {value} = this.state;return <input type="text" value={value} onChange={this.onChange} />;
}
}const stub = ReactDOM.render(<Stub />, container);
const node = ReactDOM.findDOMNode(stub);setUntrackedValue.call(node, '2.0');
dispatchEventOnNode(node, 'input');expect(node.value).toBe('2.0');
if (disableInputAttributeSyncing) {
expect(node.hasAttribute('value')).toBe(false);
} else {
...
)stub.setState({value: '0.98'});expect(node.value).toEqual('0.9class Stub extends React.Component {
state = {
value: '',
};return <input type="number" value={this.state.value} readOnly={true} />;
}
}stub.setState({value: 0});expect(node.value).toEqual('0');
  });  it('updates the value on radio buttons from "" to 0', function () {
ReactDOM.render(
<input type="radio" value="" onChange={functio);
...
<input);
ReactDOM.render(
<input type="checkbox" value={0} onChange={fun);
expect(container.firstChild.val  });  it('distinguishes precision for extra zeroes in string number values', () => {
class Stub extends React.Component {
state = {
value: '3.0000',
};
render() {
return}let stub;expect(() => {
stub = ReactDOM.render(<Stub />, container);
}).toErrorDev(
'You provided a `value` prop to a form field ' +
'without an `onChange` handler.',
...
stub.setState({value: '3'});expect(node.value).toEqual('3');
  });  it('should display `defaultValue` of number 0', () => {
const stub = <input typexpect(node.value).toBe('0');
  });  it('only assigns defaultValue if it changes', () => {
 return <input defaultValue="0" />;
}const node = ReactDOM.findDOMNode(component);Object.defineProperty(node, 'defaultValue', {
get() {},
set(value) {
throw new Error(
`defaultValue was assigned ${value}, but it did not change!`,
  ...
  });const node = ReactDOM.render(stub, container);expect(node.value).toBe('true');
  });  it('should display "false" for `defaultValue` of `false`', () => {
const stub = <input type="text" defaultValue={false} />;
const node = ReactDOM.render(stub, container);expect(node.value).toBe('false');
...
}ReactDOM.render(<input type="text" defaultValue="1" />, container);if (disableInputAttributeSyncing) {
expect(node.value).toBe('1');
expect(node.defaultValue).toBe('1');
expect(isValueDirty(node)).toBe(false);
} else {expect(node.defaultValue).toBe('1');
expec  });  it('should update `defaultValue` for uncontrolled date/time input', () => {
const node = ReactDOM.render(
<input type="date" defaultValue="1980-01-01" />,
container,
);expect(node.value).toBe('1980-01-01');
expect(node.defaultValue).toBe('1980-01-01');ReactDOM.render(<input type="date" defaultValue="2000-01-01" />, container);if (disableInputAttributeSyncing) {
expect(node.value).toBe('2000-01-01');
expect(node.defaultValue).toBe('2000-01-01');
}expect(node.defaultValue).toBe('2000-01-01');
}ReactDOM.render(<input type="date" />, container);
  });  it('should take `defaul<input type="text" value="0" readOnly={true} />,
contaexpect(node.value).toBe('0');
expect(isValueDirty(node)).toBe(true);
expect(() =>
...
<inpuconst div = document.createElement('div');
...
expect(div.firstChild.getAttribute('name')).toBe('name');
  });expect(coconst element = <input type="text" />;
const markup = ReactDOMServer.renderToString(element);
const div = document.createElement('div');
div.innerHTML = markup;
...
class TemporalLike {
valueOf() {// See https://tc39.es/proposal-temporal/docs/plaindate.html#valueOf
...
'Form field values (value, checked, defaultValue,);
  });valueOf() {
// Throwing here is the behavior of ECMAScript "Temporal" date/time API.
// See https://tc39.es/proposal-temporal/docs/plaindate.html#valueOf
throw new TypeError('prod message');
...
co<input value={new TemporalLike()} type="text" onChange={() => {}} />,
container,expect(() =>).toErrorDev(
'Form field values (value, checked, defaultValue, or defaultChecked props) must be ' +
'strings, not TemporalLike. This value must be coerced to a string before before using it here.',
);
  });  it('should display `value` of number 0', () => {
const stub = <input type="text" value={0} onChange={emptyFunction} />;
clet stub = <input type="text" value="yolo" onChange={emptyFunction} />;
ccontainer,
);
expect(node.value).toEqual('true');
  });  it('should allow setting `value` to `false`', () => {
let stub = <input type="text" value="yolo" onChange={emptyFunction} />;
const node = ReactDOM.render(stub, container);expect(node.value).toBe('yolo');stub = ReactDOM.render(
...
const node = ReactDOM.render(stub, container);expect(node.value).toBe('foo');const objToString = {
toString: function () {
retur};
...
ReactDOM.render(<input value="a" onChange={() => {}} />, container);const node = container.firstChild;
lObject.defineProperty(node, 'value', {
get: function () {
return nodeValue;
},
...
ReactDOM.render(<input value="0" onChange={() => {}} />, container);const node = container.firstChild;
let nodeValue = '0';
cget: function () {
return nodeValue;
},
...
ReactDOM.render(<input value="true" onChange={() => {}} />, container);const node = container.firstChild;
let nodeValue = 'true';
cget: function () {
return nodeValue;
},
...
const stub = <input type="text" value={0} onChange={emptyFunction} />;
const node = ReactDOM.render(stub, container);setUntrackedValue.call(node, 'giraffe');
dispatchEventOnNode(node, 'input');
expect(node.value).toBe('0');
  });const node = ReactDOM.render(stub, container);setUntrackedValue.call(node, '0.0');
dispatchEventOnNode(node, 'input');  });  it('should properly control 0.0 for a number input', () => {
cdispatchEventOnNode(node, 'input');if (disableInputAttributeSyncing) {
expect(node.value).toBe('0.0');
expect(node.hasAttribute('value')).toBe(false);
} else {
dispatchEventOnNode(node, 'blur');
dispatchEventOnNode(node, 'focusout');expect(node.value).toBe('0.0');
...
expect(isValueDirty(node)).toBe(false);ReactDOM.render(
<input type="text" value={0} onChange={emptyFunction} />,
container,
);expect(node.value).toBe('0');
expect(isValueDirty(node)).toBe(true);if (disableInputAttributeSyncing) {
expect(node.hasAttribute('value')).toBe(false);
} else {
...
const node = container.firstChild;
expect(isValueDirty(node)).toBe(true);ReactDOM.render(
<input type="text" value="" onChange={emptyFunction} />,
container,
...
container,
);const node = container.firstChild;expect(node.value).toBe('0.0');
if (disableInputAttributeSyncing) {
expect(node.hasAttribute('value')).toBe(false);
}}
...
container,
);const node = container.firstChild;expect(node.value).toBe('0');
if (disableInputAttributeSyncing) {
expect(node.hasAttribute('value')).toBe(false);
} else {
expect(node.defaultValue).toBe('0');
...
ReactDOM.render(
<input type="number" value="0" onChange={emptyFunction} />,
contaif (disableInputAttributeSyncing) {
expect(node.hasAttribute('value')).toBe(false);
} else {
expect(node.defaultValue).toBe('0');
}
...
const node = ReactDOM.render(stub, container);setUntrackedValue.call(node, 'giraffe');dispatchEventOnNode(node, 'input');expect(handled).toBe(true);
...
eexpect(isValueDirty(inputRef.current)).toBe(false);
} else {
expect(isValueDirty(inputRef.current)).toBe(true);dispatchEventOnNode(inputRef.current, 'input');
e<form>
<input defaultValue="default2" ref={inputRef} />
<input type="reset" />container,expect(inputRef.current.value).toBe('changed');
expect(isValueDirty(inputRef.current)).toBe(true);container.firstChild.reset();
...
e  });  it('should not set a value for submit buttons unnecessarily', () => {
const stub = <input type="submit" />;
ReactDOM.render(stub, container);// should have the default "Subm// the actual label that the user sees.
expect(node.hasAttribute('value')).toBe(false);
...
ReactDOM.render(stub, container);// Not really relevant to this particular test, but changing to undefined
/ReactDOM.render(
<input type="submit" value={undefined} onChange={emptyFunction} />,
container,).toErrorDev();const node = container.firstChild;
...
);const node = container.firstChild;  });  it('should set a value on a submit input', () => {
const stub = <input type="submit" value="banana" />;
ReactDOM.render(stub, container);
const node = container.firstChild;expect(node.getAttribute('value')).toBe('banana');
  });  it('should not set an undefined value on a submit input', () => {
const stub = <input type="submit" value={undefined} />;
ReactDOM.render(stub, container);
const node = container.firstChild;// Note: it shouldn't be an empty string
// because that would erase the "submit" label.
expect(node.getAttribute('value')).toBe(null);ReactDOM.render(stub, container);
expconst stub = <input type="reset" value={undefined} />;
ReactDOM.render(stub, container);// because that would erase the "reset" label.
expect(node.getAttribute('value')).toBe(null);ReactDOM.render(stub, container);
econst stub = <input type="submit" value={null} />;
expect(() => {}).toErrorDev('`value` prop on `input` should not be null');
const node = container.firstChild;// Note: it shouldn't be an empty string
// because that would erase the "submit" label.
expect(node.getAttribute('value')).toBe(null);ReactDOM.render(stub, container);
expect(node.getAttribute('value')).toBe(null);
...
conexpect(node.getAttribute('value')).toBe(null);ReactDOM.render(stub, container);
expect(node.getAttribute('value')).toBe(null);
  });ReactDOM.render(stub, container);
const node = container.firstChild;expect(node.getAttribute('value')const stub = <input type="submit" value="" />;
ReactDOM.render(stub, container);
const node = container.firstChild;expect(node.getAttribute('value')).toBe('');
...
<input
 ref={this.cRef}
 type defaultChecked={true}
 onChange={emptyFunction}
/></div>
);}const stub = ReactDOM.render(<RadioGroup />, container);
const aNode = stub.aRef.current;
const bNode = stub.bRef.current;
const cNode = stub.cRef.current;expect(aNode.checked).toBe(true);
expect(bNode.checked).toBe(false);
// c is in a separate form and shoexpect(aNode.hasAttribute('checked')).toBe(false);
...
expect(cNode.checked).toBe(true);// The original 'checked' attribute should be unchanged
if (disableInputAttributeSyncing) {
expect(aNode.hasAttribute('checked')).toBe(expect(cNode.hasAttribute('checked')).toBe(true);
} else {
expect(aNode.hasAttribute('checked')).toBe(true);
expect}// Now let's run the actual ReactDOMInput change event handler
dispatchEventOnNode(bNode, 'click');expect(cNode.checked).toBe(true);
...
exexpect(firstRadioNode.checked).toBexpect(firstRadioNode.checked).toBe(false);
...
/>
<input
type="value="2"onChange={() => {}}
/>
</div>,
container,
);
expect(container.querySelector('input[name="a"][value="1"]').checked).toBe(
true,expect(container.querySelector('input[name="b"][value="2"]').checked).toBe(
true,
);
  });  it('should control radio buttons if the tree updates during render', () => {
const sharedParent = container;
const container1 = document.createElement('div');
const container2 = document.createElement('div');sharedParent.appendChild(container1);let aNode;
let bNode;state = {changed: false};this.setState({});
...
}class ComponentB extends React.Component {return (
<div>
<input
ref={n => (bNode = n)}
type="radio"
...
}ReactDOM.render(<ComponentA />, container1);expect(aNode.checked).toBe(false);
expect(bNode.checked).toBe(true);setUntrackedChecked.call(aNode, true);
// This next line isn't necessary in a proper browser environment, but
/setUntrackedChecked.call(bNode, false);// Now let's run the actual ReactDOMInput change event handler
dispatchEventOnNode(aNode, 'click');// The original state should have been restored
expect(aNode.checked).toBe(false);
expect(bNode.checked).toBe(true);ReactDOM.render(
...
).toErrorDev('field without an `onChange` handler. This will render a read-only ' +
...
Rexpect(isValueDirty(container.firstChild)).toBe(false);
} else {
expect(isValueDirty(container.firstChReactDOM.render(<input type="text" defaultValue={''} />, container);
...
expect(() =>
ReactD'`value` prop on `input` should not be null. ' +
'Consider using an empty string to clear the component or `undefined` ' +
'forReactDOM.unmountComponentAtNode(container);ReactDOM.render(<input type="text" value={null} />, container);
  });  it('should warn if checked and defaultChecked props are specified', () => {
expect(() =>
ReactDOM.render(type="radio"
checkreadOnly={true}
/>,
container,
),'A component contains an input of type radio with both checked and defaultChecked props. ' +
'Input elements must be either controlled or uncontrolled ' +
'(specify either the checked prop, or the defaultC'element and remove one of these props. More info: ' +
...
'both). Decide between using a control'https://reactjs.org/link/controlled-components',
);
ReactDOM.unmountComponentAtNode(container);container,
);
...
'This is likely caused by the value ch'Decide between using a controlled or uncontrolled input ' +
'element for the lifetime of the component. More info: https://reactjs.org/link/controlled-components\n' +
'in input (at **)',  });  it('should warn if controlled input switches to uncontrolled (value is null)', () => {
const stub = (
<inpuReactDOM.render(stub, container);
expect(() =>
ReactDOM.render(<input type="text" value={null} />, container),
).toErrorDev(['Consider using an empty string to clear the component or `undefined` for uncontrolled components',
'Warn'undefined, which should not happen. ' +
'Decide between using a controlled or uncontrolled input ' +
'element for the lifetime of the component. More info: https://reactjs.org/link/controlled-components\n' +
'in input (at **)',  });  it('should warn if controlled input switches to uncontrolled with defaultValue', () => {
c);
ReactDOM.render(stub, container);
expect(() =>
ReactDOM.render(container,
...
'Warning: A component is changing a controlled input to be uncontrolled. ' +
'This is likely caused by the value changing from a defined to ' +
'undefined, which should not happen. 'element for the lifetime of the component. More info: https://reactjs.org/link/controlled-components\n' +
'in input (at **)',
);
...
e).toErrorDev(
'Warning: A component is changing a controlled input to be uncontrolled. ' +
'This is likely caused by the value c'Decide between using a controlled or uncontrolled input ' +
'element for the lifetime of the component. More info: https://reactjs.org/link/controlled-components\n' +
'in input (at **)',
);
...
ReactDOM.render(stub, container);
e<input type="checkbox" defaultChecked={true} />,
container,).toErrorDev(
'Warning: A component is changing a controlled input to be uncontrolled. ' +
'This is likely caused by the value changing from a defined to ' +
'unde'element for the lifetime of the component. More info: https://reactjs.org/link/controlled-components\n' +
'in input (at **)',
);
...
container,
);<input type="radio" value="value" onChang);
...
let value = '';
let typeProp = '';if (type === 'input') {
Objecreturn typeProp;set: function (val) {
typeProp = String(val);},
});
Object.defineProperty(el, 'value', {
get: function () {
return value;set: function (val) {
value = String(val);},
});
...
state = {type: 'number', value: 1000};render() {
const }
}const input = ReactDOM.render(<Input />,// the value will not be assigned.
...
HTMLInputElement.prototype,
'defaconst getValue = Object.getOwnPropertyDescriptor(
HTMLInputElement.prototype,
'value',
).get;
...
const getType = Object.getOwnPropertyDescriptor'type',
).getHTMLInputElement.prototype,
'type',
).set;
if (type === 'input') {
...
return el;
});ReactDOM.render(<input type="date" defaultValue="1980-01-01" />, container);if (disableInputAttributeSyncing) {
expect(log).toEqual([
'node.type = "date"',
...
const node = ReactDOM.findDOMNode(stub);
expect(isValueDirty(node)).toBe(false);setUntrackedValue.call(node, '2');
dispatchEventOnNode(node, 'input');expect(isValueDirty(node)).toBe(true);
if (disableInputAttributeSyncing) {
expect(node.hasAttribute('value')).toBe(false);
} else {
...
container,
);expect(isValueDirty(node)).toBe(true);node.focus();setUntrackedValue.call(node, '2');
dispatchEventOnNode(node, 'input');expect(isValueDirty(node)).toBe(true);
if (disableInputAttributeSyncing) {
expect(node.hasAttribute('value')).toBe(false);
} else {
...
const node = ReactDOM.findDOMNode(stub);
expect(isValueDirty(node)).toBe(true);node.focus();
setUntrackedValue.call(node, '2');
dispatchEventOnNode(node, 'input');
node.blur();expect(isValueDirty(node)).toBe(true);
if (dexpect(node.hasAttribute('value')).toBe(false);
} else {
...
expecsetUntrackedValue.call(node, 4);
dispatchEventOnNode(node, 'input');
node.blur();expect(isValueDirty(node)).toBe(true);
expect(node.getAttribute('value')).toBe('1');
...
let input;function renderInputWithStringThenWithUndefined() {
...
let input;function renderInputWithStringThenWithNull() {
let setValueToNull;
class Input extends React.Component {
constructor() {
super();
...
const node = container.firstChild;expect(node.value).toBe('');
if (disableInputAttributeSyncing) {
expect(node.hasAttribute('value')).toBe(false);
} else {
...
Reactexpect(node.getAttribute('value')).toBe('');
...
const node = container.firstChild;if (disableInputAttributeSyncing) {
expect(node.value).toBe('');
} else {
...
const// TODO: we should warn here.
...
const node = container.firstChild;expect(node.value).toBe('on');
expect(node.hasAttribute('value')).toBe(false);
...
expect(node.defaultValue).toBe('0');ReactDOM.render(<input type="text" />, container);
expect(node.defaultValue).toBe('');
...
expect(node.defaultValue).toBe('0');ReactDOM.render(<input type="text" defaultValue={null} />, container);
expect(node.defaultValue).toBe('');
...
contaReactDOM.render(
<input type="text" value="a" onChange={onChange} />,
container,
);const node = container.firstChild;
setUntrackedValue.call(node, '');
dispatchEventOnNode(node, 'input');expect(log).toEqual(['']);
