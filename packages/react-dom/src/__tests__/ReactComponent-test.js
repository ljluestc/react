...
let ReactDOM;
let ReactDOMServer;
let ReactTestUtils;describe('ReactComponent', () => {
  beforeEach(() => {
...
}).toThrowError(/Target container is not a DOM element./);expect(function () {
...
const log = [];class Inner extends React.Component {
render() {
...
ReactDOM.unmountComponentAtNode(el);/* eslint-disable indent */
expect(log).toEqual([
'start mount',
'inner 1 render',
'inner 2 render',
...
const X = undefined;function Indirection(props) {
return <div>{props.children}</div>;
}function Bar() {
return (
<Indirection>
