 ...
 *
 * @emails react-core
 */  let ReactDO  let act;  function resetModules() {
({ReactDOM, ReactDOMServer} = initModules());  }  function shouldUseDocument(reactElement) {
// Used for whole document tests.
return reactElement && reactElement.type === 'html';
  }  functconst doc = document.implementation.createHTMLDocument('');
doc.open();
doc.write(
mar);
doc.close();
return doc;
} econtainer.innerHTML = markup;
return container;
}
...
ReactDOM.render(reactElement, domElement);
  ...
  async function expectErrors(fn, count) console.error.mockClear();
} else {
...
err  }  // Renders text using SSR and then stuffs it into a DOM node; returns the DOM
...
resetModules();const container = getContainerFromMarkup(element, markup);
let serverNode = container.firstChild;const firstClientNode = await renderIntoDom(
element,
container,
true,
errorCount,
...
exp// Assert that the DOM element hasn't been replaced.
...
return firstClientNode;
  };  function BadMarkupExpected() {}  const clientRenderOnBadMarkup = async (element, errorCount = 0) => {
// First we render the top of bad mark up.const container = getContainerFromMarkup(
element,
sho: '<div id="badIdWhichWillCauseMismatch"></div>',
);await renderIntoDom(element, container, true, errorCount + 1);// This gives us the resulting text content.
const hydratedTextContent =
container.lastChild && container.lastChild.textContent;// Next we render the element into a clean DOM node client side.
let cleanContainer;
if (shouldUseDocument(element)) {
...
cleanContainer = document.createElemenawait asyncReactDOMRender(element, cleanContainer, true);
// This gives us the expected text content.
const cleanTextContent =
(cleanContainer.lastChild && cleanContainer.lastChild.textContent) || '';// The only guarantee is that text content has been patched up if needed.
...
throw new BadMarkupExpected();
  };  // runs a DOM rendering tes  // -- render to string on server
...
it(`itClientRenders(desc, testFn);  // -- render on client without any server markup "clean client render"
  // -- render on client on top of good se  //
  // testFn is a test that has one arg, which is a render function. the render
  // function takes in a ReactElement and an optional expected error count and
  // returns a promise of a DOM Element.
  //
function itClientRenders(desc, testFn) {testFn(clientCleanRender));
it(`renders ${desc} with client render on top of good server markup`, () =>
testFn(clientRenderOnServerString));try {
