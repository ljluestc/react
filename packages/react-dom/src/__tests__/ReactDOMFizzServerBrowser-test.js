...
global.TextEncoder = require('util').TextEncoder;let React;
let ReactDOMFizzServer;
let Suspense;describe('ReactDOMFizzServerBrowser', () => {
  beforeEach(() => {
jest.resetModules();
React = require('react');
...
);stream.allReady.then(() => (isComplete = true));await jest.runAllTimers();
expect(isComplete).toBe(false);
// Resolve the loading.
hasLoaded = true;
await resolve();await jest.runAllTimers();expect(isComplete).toBe(true);const result = await readResult(stream);
expect(result).toMatchInlineSnapshot(
`"<div><!--$-->Done<!-- --><!--/$--></div>"`,
);
...
);controller.abort();const result = await readResult(stream);
expect(result).toContain('Loading');expect(errors).toEqual(['The operation was aborted.']);
...
);await jest.runAllTimers();const theReason = new Error('aborted for reasons');
controller.abort(theReason);let caughtError = null;
try {
await promise;
} catch (error) {
caughtError = error;
...
controller.abort(theReason);const promise = ReactDOMFizzServer.renderToReadableStream(
<div>
<Suspense fallback={<div>Loading</div>}>
<InfiniteSuspend />
</Suspense>
...
);stream.allReady.then(() => (isComplete = true));expect(rendered).toBe(false);
expect(isComplete).toBe(false);const reader = stream.getReader();
reader.cancel();expect(errors).toEqual([
'The render was aborted by the server without a reason.',
]);hasLoaded = true;
resolve();await jest.runAllTimers();expect(rendered).toBe(false);
...
});controller.abort('foobar');expect(errors).toEqual(['foobar', 'foobar']);
...
});controller.abort(new Error('uh oh'));expect(errors).toEqual(['uh oh', 'uh oh']);
