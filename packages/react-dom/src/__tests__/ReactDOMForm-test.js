 ...
 *
 * @emails react-core
 */'use strict';global.IS_REACT_ACT_ENVIRONMENT = true;// Our current version of JSDOM doesn't implement the event dispatching
// so we polyfill it.
...
global.FormData = FormDataPolyfill;describe('ReactDOMForm', () => {
  let act;
  let container;
  let React;
  let ReactDOM;
...
document.body.appendChild(container);textCache = new Map();
...
});await submit(ref.current);expect(foo).toBe('bar');// Try updating the actionfunction action2(formData) {
...
});await submit(ref.current);expect(foo).toBe('bar2');
  });  // @gate enableFormActions
...
expect(deletedTitle).toBe(null);await submit(inputRef.current);
expect(savedTitle).toBe('Hello');
expect(deletedTitle).toBe(null);
savedTitle = null;await submit(buttonRef.current);
expect(savedTitle).toBe(null);
expect(deletedTitle).toBe('Hello');
deletedTitle = null;// Try updating the actionsfunction saveItem2(formData) {
...
savedTitle = null;await submit(buttonRef.current);
expect(savedTitle).toBe(null);
expect(deletedTitle).toBe('Hello2');expect(rootActionCalled).toBe(false);
  });  // @gate enableFormActions || !__DEV__
  it('should allow preventing default to block the action', async () => {
const ref = React.createRef();
let actionCalled = false;function action(formData) {
actionCalled = true;
}const root = ReactDOMClient.createRoot(container);
await act(async () => {
root.render(
...
});await submit(ref.current);expect(actionCalled).toBe(false);
  });  // @gate enableFormActions
  it('should only submit the inner of nested forms', async () => {
const ref = React.createRef();
let data;function outerAction(formData) {
...
'\nin form (at **)' +
'\nin form (at **)',
]);await submit(ref.current);expect(data).toBe('innerinner');
  });  // @gate enableFormActions
  it('should only submit once if one root is nested inside the other', async () => {
const ref = React.createRef();
let outerCalled = 0;
let innerCalled = 0;
let bubbledSubmit = false;function outerAction(formData) {
outerCalled++;
}function innerAction(formData) {
innerCalled++;
...
});await submit(ref.current);expect(bubbledSubmit).toBe(true);
...
});innerContainerRef.current.appendChild(innerContainer);await submit(ref.current);expect(bubbledSubmit).toBe(true);
expect(outerCalled).toBe(0);
expect(innerCalled).toBe(1);
  });  // @gate enableFormActions
...
const buttonRef = React.createRef();
let button;
let title;function action(formData) {
...
});await submit(inputRef.current);expect(button).toBe('delete');
expect(title).toBe(null);await submit(buttonRef.current);expect(button).toBe('edit');
expect(title).toBe('hello');// Ensure that the type field got correctly restored
...
const inputRef = React.createRef();
const buttonRef = React.createRef();
let button;function action(formData) {
...
]);await submit(inputRef.current);expect(button).toBe(null);await submit(buttonRef.current);expect(button).toBe(null);// Ensure that the type field got correctly restored
expect(inputRef.current.getAttribute('type')).toBe('submit');
expect(buttonRef.current.getAttribute('type')).toBe(null);
  });  // @gate enableFormActions || !__DEV__
...
const formRef = React.createRef();function Status() {
...
assertLog(['Initial']);
expect(container.textContent).toBe('Initial');// This should suspend because form actions are implicitly wrapped
// in startTransition.
await submit(formRef.current);
assertLog(['Pending...', 'Suspend! [Updated]', 'Loading...']);
expect(container.textContent).toBe('Pending...Initial');await act(() => resolveText('Updated'));
assertLog(['Updated']);
...
await act(() => root.render(<App />));
assertLog(['Count: 0']);
expect(container.textContent).toBe('Count: 0');// Update
await submit(formRef.current);
assertLog(['Pending...', 'Suspend! [Count: 1]', 'Loading...']);
expect(container.textContent).toBe('Pending...Count: 0');await act(() => resolveText('Count: 1'));
assertLog(['Count: 1']);
expect(container.textContent).toBe('Count: 1');// Update again
await submit(formRef.current);
assertLog(['Pending...', 'Suspend! [Count: 2]', 'Loading...']);
expect(container.textContent).toBe('Pending...Count: 1');await act(() => resolveText('Count: 2'));
assertLog(['Count: 2']);
...
await act(() => root.render(<App />));
assertLog(['Initial']);
expect(container.textContent).toBe('Initial');await submit(formRef.current);
assertLog(['Async action started', 'Pending...']);await act(() => resolveText('Wait'));
...
}const formRef = React.createRef();function App() {
return (
<ErrorBoundary>
<form
action={() => {
...
expect(container.textContent).toBe('Everything is fine');await submit(formRef.current);
...
expect(container.textContent).toBe('Everything is fine');await act(() => resolveText('Wait'));
assertLog(['Oh no!', 'Oh no!']);
expect(container.textContent).toBe('Oh no!');
  });  // @gate enableFormActions
  // @gate enableAsyncActions
...
expect(container.textContent).toBe('No pending action');await submit(formRef.current);
assertLog([
