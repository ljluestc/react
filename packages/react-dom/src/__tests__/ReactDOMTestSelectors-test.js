 ...
 *
 * @emails react-core
 */'use strict';describe('ReactDOMTestSelectors', () => {
  let React;  let act;
  ...
  let render;  let container;  beforeEach(() => {
jest.resetModules();React = require('react');
act = React.unstable_act;if (__EXPERIMENTAL__ || global.__WWW__) {
...
}render(<Example />, container);const matches = findAllNodes(document.body, [
...
}render(<Example />, container);const matches = findAllNodes(container, [
createComponentSelector(Example),
createTestNameSelector('match'),
]);
expect(matches).toHaveLength(1);
...
}render(<Outer />, container);let matches = findAllNodes(container, [
createComponentSelector(Outer),
createTestNameSelector('outer'),
]);
expect(matches).toHaveLength(1);
expect(matches[0].id).toBe('outer');matches = findAllNodes(matches[0], [
...
return <div id="inner" />;
}render(<Outer />, container);const matches = findAllNodes(container, [createComponentSelector(Outer)]);
expect(matches).toHaveLength(1);
expect(matches[0].id).toBe('outer');expect(() => {
...
}render(<Outer />, container);let matches = findAllNodes(document.body, [
createComponentSelector(Outer),
createComponentSelector(Middle),
createTestNameSelector('match'),
]);
expect(matches).toHaveLength(2);
expect(matches.map(m => m.id).sort()).toEqual(['match2', 'match3']);matches = findAllNodes(document.body, [
createComponentSelector(Outer),
createComponentSelector(Middle),
createComponentSelector(Inner),
createTestNameSelector('match'),
...
expect(matches).toHaveLength(1);
expect(matches[0].id).toBe('match3');matches = findAllNodes(document.body, [
createComponentSelector(Outer),
createComponentSelector(Inner),
createTestNameSelector('match'),
]);
...
const ref2 = React.createRef(null);function Outer() {
return (
<>
<div hidden={true}>
...
}render(<Outer />, container);const matches = findAllNodes(document.body, [
createComponentSelector(Outer),
createTestNameSelector('match'),
]);expect(matches).toHaveLength(1);
...
const ref = React.createRef();function Example() {
return (
<div>
<div>foo</div>
<div>
...
function Example() {
return <div ref={ref} />;
}render(<Example />, container);expect(() => findAllNodes(ref.current, [])).toThrow(
'Invalid host root specified. Should be either a React container or a node with a testname attribute.',
);
});
...
}render(<Outer />, container);const description = getFindAllNodesFailureDescription(document.body, [
createComponentSelector(Outer),
createComponentSelector(Middle),
...
`findAllNodes was able to match part of the selector:
  <Outer> > <Middle>No matching component was found for:
...
}render(<Example />, container);const description = getFindAllNodesFailureDescription(document.body, [
createComponentSelector(Example),
...
}render(<Example />, container);setBoundingClientRect(ref.current, {
x: 10,
y: 20,
width: 200,
height: 100,
...
const innerRef = React.createRef();function Outer() {
return (
<>
<div ref={outerRef} />
<Inner />
...
}render(<Outer />, container);setBoundingClientRect(outerRef.current, {
x: 10,
y: 20,
width: 200,
height: 100,
...
const refB = React.createRef();function Example() {
return (
<>
<div ref={refA}>
<div />
...
}render(<Example />, container);setBoundingClientRect(refA.current, {
x: 10,
y: 20,
width: 200,
height: 100,
...
const refC = React.createRef();function Example() {
return (
<>
<div ref={refA} />
<div ref={refB} />
...
const refG = React.createRef();function Example() {
return (
<>
...
}render(<Example />, container);// A, B, and C are all adjacent and/or overlapping, with the same height.
...
function NotUsed() {
return null;
}render(<Example />, container);const didFocus = focusWithin(document.body, [
createComponentSelector(Example),
...
const secondRef = React.createRef(null);const handleFirstFocus = jest.fn();
const handleSecondFocus = jest.fn();
const handleThirdFocus = jest.fn();function Example() {
return (
<>
<FirstChild />
<SecondChild />
...
const ref = React.createRef(null);const handleFocus = jest.fn(event => {
event.target.blur();
});function Example() {
return (
<button
...
let observedTargets;beforeEach(() => {
callback = null;
observedTargets = [];class IntersectionObserver {
constructor() {
...
const ref = React.createRef(null);function Example() {
return <div ref={ref} />;
}render(<Example />, container);// Stub out the size of the element this test will be observing.
const rect = {
x: 10,
y: 20,
width: 200,
...
setBoundingClientRect(ref.current, rect);const handleVisibilityChange = jest.fn();
observeVisibleRects(
document.body,
[createComponentSelector(Example)],
...
expect(handleVisibilityChange).not.toHaveBeenCalled();// Simulate IntersectionObserver notification.
simulateIntersection([ref.current, rect, 0.5]);expect(handleVisibilityChange).toHaveBeenCalledTimes(1);
...
const ref1 = React.createRef(null);
const ref2 = React.createRef(null);function Example() {
return (
<>
<div ref={ref1} />
<div ref={ref2} />
...
setBoundingClientRect(ref2.current, rect2);const handleVisibilityChange = jest.fn();
observeVisibleRects(
document.body,
[createComponentSelector(Example)],
handleVisibilityChange,
...
expect(handleVisibilityChange).not.toHaveBeenCalled();// Simulate IntersectionObserver notification.
simulateIntersection([ref1.current, rect1, 0.5]);// Even though only one of the rects changed intersection,
...
{rect: rect2, ratio: 0},
]);handleVisibilityChange.mockClear();rect2 = {
x: 210,
y: 20,
width: 200,
height: 200,
...
expect(observedTargets).toHaveLength(1);
expect(handleVisibilityChange).not.toHaveBeenCalled();disconnect();
...
const ref1 = React.createRef(null);
const ref2 = React.createRef(null);let increment;function Example() {
...
setBoundingClientRect(ref1.current, rect1);const handleVisibilityChange = jest.fn();
observeVisibleRects(
document.body,
[createComponentSelector(Example)],
handleVisibilityChange,
);// Simulate IntersectionObserver notification.
simulateIntersection([ref1.current, rect1, 1]);expect(handleVisibilityChange).toHaveBeenCalledTimes(1);
expect(handleVisibilityChange).toHaveBeenCalledWith([
{rect: rect1, ratio: 1},
]);act(() => increment());const rect2 = {
x: 110,
y: 20,
width: 200,
height: 100,
};
setBoundingClientRect(ref2.current, rect2);handleVisibilityChange.mockClear();simulateIntersection(
...
{rect: rect1, ratio: 0.5},
{rect: rect2, ratio: 0.25},
]);act(() => increment());handleVisibilityChange.mockClear();simulateIntersection([ref2.current, rect2, 0.75]);expect(handleVisibilityChange).toHaveBeenCalledTimes(1);
expect(handleVisibilityChange).toHaveBeenCalledWith([
