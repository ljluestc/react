...
let waitForAll;describe('ReactDOM HostSingleton', () => {
  beforeEach(() => {
jest.resetModules();
...
waitForAll = InternalTestUtils.waitForAll;// Test Environment
...
const persistentElements = [documentElement, head, body];// Render into the document completely different html. Observe that styles
...
hydrateRoot.unmount();// Now we try hydrating again with matching nodes and we ensure
...
await waitForAll([]);// We construct and insert some artificial stylesheets mimicing what a 3rd party script might do
...
head.appendChild(c);
head.appendChild(d);const bodyContent = document.body.firstChild;
const body = document.body;
body.insertBefore(e, bodyContent);
body.insertBefore(f, bodyContent);
body.appendChild(g);
body.appendChild(h);expect(getVisibleChildren(document)).toEqual(
<html>
<head>
...
container = document.documentElement;const root = ReactDOMClient.createRoot(container);
root.render(
<>
<head>
...
container = document.head;const root = ReactDOMClient.createRoot(container);
...
container = document.body;const root = ReactDOMClient.createRoot(container);
...
);root.unmount();root = ReactDOMClient.createRoot(document);
root.render(
<html>
<head />
<body>baz</body>
