 ...
 *
 * @emails react-core
 */let ReactDOM;describe('ReactLegacyErrorBoundaries', () => {
  let log;  let BrokenConstructor;
  let BrokenComponentWillMount;
  let BrokenComponentD  let BrokenComponentWillUpdate;
  let BrokenComponentDidUpdate;
  let Brok  let BrokenComponentWillMountErrorBoundary;
  ...
  let Normal;  beforeEach(() => {
jest.resetModules();
PropTypes = require('prop-types');
ReactFeatureFlags = require('shared/ReactFeatureFlags');
ReactFeatureFlags.replayFailedUnitOfWorkWithInvokeGuardedCallback = false;
ReactDOM = require('react-dom');
React = require('react');log = [];BrokenConstructor = class extends React.Component {
constructor(props) {
super(props);throw new Error('Hello');
}
render() {
log.push('BrokenConstructor render');
return <div>{this.props.children}</div>;
}
UNSAFE_componentWillMount() {}log.push('BrokenConstructor componentDidMount');
}
...
componentDidMount() {
log.puUNSAFE_componentWillReceiveProps() {
...
renderlog.push(`${this.props.logName} render error`);
...
}).toThrow('Hello');container = document.createElement('div');
...
const container3 = document.createElement('div');ReactDOM.render(<span>Before 1</span>, container1);
expect(() => {
...
expect(container3.firstChild.textContent).toBe('Caught an error: Hello.');ReactDOM.render(<span>After 1</span>, container1);
...
expect(container3.firstChild.textContent).toBe('After 3');ReactDOM.unmountComponentAtNode(container1);
...
const container = document.createElement('div');
s<BothErrorBoundaries>
<BrokenRender />
</Bot);
if (__DEV__) {
expect(console.error).toHaveBeenCalledTimes(2);
expect(console.error.mock.calls[0][0]).toContain(
'ReactDOM.render is no lexpect(console.error.mock.calls[1][0]).toContain(
'The above error occurred in the <BrokenRender> component:',
);
}expect(containe'BothErrorBoundaries constructor',
'BothErrorBoundaries componentWillMount',
'BothErrorBoundaries render success',
'BrokenRender constructor',
'Brok// Both getDerivedStateFromError and componentDidCatch should be called
...
'BothErrorBoundaries com'BothErrorBoundaries componentDidCatch',
'BothErrorBoundaries componentWillUpdate',
'BothErrorBoundaries render error',
'BothErrorBoundaries componentDidUpdate',
]);log.length =expect(log).toEqual(['BothErrorBoundaries componentWillUnmount']);
...
'Erro'BrokenRender componentWillMount',
'BrokenRender render [!]',
// Fiber mounts with null children before capturing error
'ErrorBoundary componentDidMount','ErrorBoundary componentDidCatch',
...
ReactDOM.unmountComponentAtNode(container);
econst container = document.createElement('div');
ReactDOM.render(
<ErrorBoundary>
...
).toErr'returns a class instance. ' +
...
'BrokenRender render [!]',
'ErrorBoundary componentDidMount',
'ErrorB'ErrorBoundary render error',
'ErrorMessage constructor',
'ErrorMessage componentWillMount',
'Erro'ErrorBoundary componentDidUpdate',
]);log.length = 0;
ReactDOM.unmountComponentAtNode(container);
expect(log).toEqual([
...
// Render first child
'Normal constructor',
'Norm// Render second child (it throws)
'BrokenRender constructor',
'BrokenRender componentWillMount',
'BrokenRender render [!]',
...
'BrokenRender constructor',
'BrokenRender componentWillMount',
'Broken// Finish mounting with null children
'ErrorBoundary componentDidMount',
// Handle the error
'Erro'ErrorBoundary componentWillUpdate',
'ErrorBoundary render error',
...
const errorMessageRef = React.createRef();const container = document.createElement('div');
ReactDOM.render(
<ErrorBoundary errorMessageRef={errorMessageRef}>
<div ref={childRef} />
<BrokenRender />
...
expect(container.textContent).toBe('Caught an error: Hello.');
exp'ErrorBoundary componentWillMount',
'ErrorBoundary render success',
'BrokenRender constructor',
'Brok// Handle error:
// Finish mounting with null children
...
Rea  });  it('catches if child throws in componentWillReceiveProps during update', () => {
const container = document.createElement('div');
ReactDOM.render(
<ErrorBoundary>
<Normal />
...
ReactDOM.render(
<ErrorBoundary><BrokenComponentWillReceiveProps />
</ErrorBoundary>,
container,
);
expect(container.textContent).toBe('Caught an error: Hello.');
...
'Normal componentWillUpdate',
'Normal render',
// Brok'BrokenComponentWillUpdate componentWillUpdate [!]',
// Finish updating with null children
'Normal componentWillUnmount',
'Brok// Handle the error
'ErrorBoundary componentDidCatch',
'ErrorBoundary componentWillUpdate',
'ErrorBoundary render error',
'ErrorBoundary componentDidUpdate',
...
'BrokenRender constructor',
'BrokenRender componentWillMount',
'Broken'Child1 ref is set to null',
'ErrorBoundary componentDidUpdate',
// Handle the error
'Erro'ErrorBoundary render error',
'Error message ref is set to [object HTMLDivElement]',
...
expect(log).toEqual([
'ErrorBoundary componentWillUnmount',
'Error  });  it('recovers from componentWillUnmount errors on update', () => {
const container = document.createElement('div');
ReactDOM.render(
<ErrorBoundary>
...
// Fibe// The components have updated in this phase
'BrokenComponentWillUnmount componentDidUpdate',
'ErrorBoundary componentDidUpdate',
// No'ErrorBoundary componentDidCatch',
...
'ErrorBoundary componentDidCatch',
'ErrorBoundary componentWillUpdate',
// Ren// Attempt to unmount previous child:
// Done
'ErrorBoundary componentDidUpdate',
]);log.length = 0;
ReactDOM.unmountComponentAtNode(container);
...
expect(log).toEqual([
'ErrorBoundary componentWillReceiveProps',
'ErrorB// Update existing children:
'Normal componentWillReceiveProps',
'Normal componentWillUpdate',
'Norm'BrokenComponentWillUnmount componentWillUpdate',
'BrokenComponentWillUnmount render',
// Unmounting throws:
'BrokenComponentWillUnmount componentWillUnmount [!]',
...
'Error'BrokenComponentWillUnmount componentWillUnmount [!]',
...
expect(log).toEqual([
// Up'OuterErrorBoundary componentWillUpdate',
...
// Now that commit phase is done, Fiber handles errors
// Only'InnerErrorBoundary componentWillUpdate',
// Render an error now
...
expect(container.textContent).toBe('Caught an error: Hello.');// Force the success path:
log.length = 0;
Rea<Normal />
</ErrorBoundary>,
container,
...
expect(log).toEqual([
'Erro'ErrorBoundary render success',
// Mount children:
'Normal constructor',
...
Re'ErrorBoundary componentWillUnmount',
'Normal componentWillUnmount',
]);
...
expect(container.textContent).toBe('Caught an error: Hello.');ReactDOM.render(
<ErrorBoundary>
<BrokenRender />
</ErrorBoundary>,
container,
);
expect(container.textContent).toBe('Caught an error: Hello.');ReactDOM.render(<div>Other screen</div>, container);
expect(container.textContent).toBe('Other screen');ReactDOM.unmountComponentAtNode(container);
...
expect(container.textContent).toBe('Caught an error: Hello.');log.length = 0;
ReactDOM.unmountComponentAtNode(container);
expect(log).toEqual(['ErrorBoundary componentWillUnmount']);
...
eexpect(log).toEqual(['ErrorBoundary componentWillUnmount']);
...
elements.push(<MaybeBrokenRender key={100} />);let currentIndex = elements.length;
while (0 !== currentIndex) {
const randomIndex = Math.floor(Math.random() * currentIndex);
currenelements[currentIndex] = elements[randomIndex];
...
expect(container.textContent).not.toContain('Caught an error');fail = true;
ReactDOM.render(
<ErrorBoundary>{getAMixOfNormalAndBrokenRenderElements()}</ErrorBoundary>,
container,
);
...
state = {shouldThrow: false};render() {
if (fail) {
...
'Stat'ErrorBoundary componentWillUpdate',
'ErrorBoundary render error',
'ErrorBoundary componentDidUpdate',
]);log.length = 0;
ReactDOM.unmountComponentAtNode(container);
...
expect(log).toEqual([
'ErrorBoundary constructor',
'Error'BrokenComponentWillUnmount constructor',
...
Rea  });  it('catches errors in componentDidUpdate', () => {
const container = document.createElement('div');
ReactDOM.render(
<ErrorBoundary>
...
expect(log).toEqual([
'Error'ErrorBoundary render success',
...
'ErrorBoundary componentDidUpdate',
'BrokenComponentDidUpdate componentWillUnmount','ErrorBoundary componentDidCatch',
'ErrorBoundary componentWillUpdate',
'ErrorBoundary render error',
'ErrorBoundary componentDidUpdate',
]);log.length = 0;
...
// Fi'BrokenComponentDidMountErrorBoundary componentWillUnmount',
// The error propagates to the higher boundary
'ErrorBoundary componentDidCatch',
// Fiber retries from the root
'ErrorBoundary componentWillUpdate',
...
ReactDOM.unmountComponentAtNode(container);  });  it('calls componentDidCatch for each error that is captured', () => {
...
}const container = document.createElement('div');<ErrorBoundary logName="OuterErrorBoundary">
<ErrorBoundaryrenderError={renderUnmountError}>
<Brok</ErrorBoundary>
<ErrorBoundary
...
/>
<ErrorBoundary
logNa<BrokenComponentDidUpdate errorText="E3" />
<BrokenComponentDidUpdate errorText="E4" />
</ErrorBoundary>
</ErrorBoundary>,
container,
...
'OuterErrorBoundary componentWillUpdate','InnerUnmountBoundary componentWillReceiveProps',
'InnerUnmountBoundary componentWillUpdate',
'InnerUnmountBoundary render success',
'Inne'InnerUpdateBoundary render success',
// First come the updates
'BrokenComponentDidUpdate componentWillReceiveProps',
'BrokenComponentDidUpdate componentWillUpdate',
'BrokenComponentDidUpdate render',
'BrokenComponentDidUpdate componentWillReceiveProps',
'BrokenComponentDidUpdate componentWillUpdate',// We're in commit phase now, deleting
'BrokenComponentWillUnmount componentWillUnmount [!]',
'BrokenComponentWillUnmount componentWillUnmount [!]',
...
'BrokenComponentDidUpdate componentDidUpdate [!]',
'Inne// After the commit phase, attempt to recover from any errors that
// were captured
...
'Inne'InnerUnmountBoundary render error',
'InnerUpdateBoundary componentWillUpdate',
'InnerUpdateBoundary render error',
'InnerUnmountBoundary componentDidUpdate',
'InnerUpdateBoundary componentDidUpdate',
]);log.length = 0;
ReactDOM.unmountComponentAtNode(container);'OuterErrorBoundary componentWillUnmount',
...
const X = null;let err1;
let err2;try {
const container = document.createElement('div');
expec'(for built-in components) or a class/function ' +
...
'NoopEr'BrokenRender constructor',
'BrokenRender componentWillMount',
'BrokenRender render [!]',
// In'NoopErrorBoundary componentDidCatch',
// Nothing happens.
]);log.length = 0;
ReactDOM.unmountComponentAtNode(container);
...
let caughtError;
class Parent extends React.Component {
getSnapthrow new Error('parent sad');
}
componentDidUpdate() {}
rende}
}
