...
let ReactDOM;
let ReactDOMServer;// In standard React, TextComponent keeps track of different Text templates
...
let nodes = filterOutComments(inst.childNodes);const foo = nodes[1];
const bar = nodes[2];
expect(foo.data).toBe('foo');
expect(bar.data).toBe('bar');inst = ReactDOM.render(
<div>
<span />
{'baz'}
{'qux'}
...
const childDiv = childNodes[1];inst = ReactDOM.render(
<div>
{null}
<div />
{null}
...
expect(childNodes[0]).toBe(childDiv);inst = ReactDOM.render(
<div>
{'foo'}
<div />
{'bar'}
...
el,
);inst.normalize();inst = ReactDOM.render(
<div>
{'bar'}
{'baz'}
{'qux'}
...
el.innerHTML = ReactDOMServer.renderToString(reactEl);ReactDOM.hydrate(reactEl, el);
expect(el.textContent).toBe('foobarbaz');ReactDOM.unmountComponentAtNode(el);reactEl = (
<div>
{''}
{''}
{''}
