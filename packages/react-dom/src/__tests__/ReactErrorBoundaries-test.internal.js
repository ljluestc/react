 ...
 *
 * @emails react-core
 */'use strict';let PropTypes;
let React;
let ReactDOM;
let act;
let ReactFeatureFlags;
let Scheduler;describe('ReactErrorBoundaries', () => {
...
assertLog = InternalTestUtils.assertLog;BrokenConstructor = class extends React.Component {
constructor(props) {
super(props);
...
Scheduler.log('BrokenUseEffect render');React.useEffect(() => {
Scheduler.log('BrokenUseEffect useEffect [!]');
throw new Error('Hello');
});return children;
};BrokenUseLayoutEffect = ({children}) => {
Scheduler.log('BrokenUseLayoutEffect render');React.useLayoutEffect(() => {
...
expect(container.textContent).toBe('Caught an error: Hello.');Scheduler.unstable_clearLog();
ReactDOM.unmountComponentAtNode(container);
assertLog(['ErrorBoundary componentWillUnmount']);
...
const container = document.createElement('div');const thrownError = new Error('original error');
const Throws = () => {
throw thrownError;
...
const container = document.createElement('div');const evilError = {
