 ...
 *
 * @emails react-core
 */  const ret =if (!obj.hasOwnProperty(name)) {
continue;if (obj[name] !== null && obj[name] !== undefined) {
ret[name] = obj[name];
}
  }
  return ret;
...
}  getInternalStat  }  componentDidMount() {
this.props.onFlush();
  }  componentDidUpdate() {
this.props.onFlush();
  }  render() {
 ...
 */
cla* Gets the order directly from each rendered child's `index` field.
* Refs are not maintained in the rendered order, and neither is
getOriginalKeys() {
const originalKeys = [];
fororiginalKeys.push(key);
}
}  }  /**
...
expect(nonEmptyStatusDisplays[username].getStatus()).toEqual(
n  }  expect(Object.keys(nonEmptyStatusDisplays)).toEqual(
...
conconst statusDisplay = statusDisplays[username];
...
container,
  ...
  verifyStatuses(statusDisplays, sequence[0]);  for (let i = 1; i < sequence.length; i++) {
ReactDOM.render(
...
v}function prepareChildrenArray(childrenArray) {
...
prepareChildrenModernIterable,
  ...
  beforeEach(() => {
jest.resetModules();
  });  it('should reset internal state if remoconst props = {
usernameToStatus: {
jcw: 'jcwStatus',
},
};const container = document.createElement('div');
...
/>,
container,
);const startingInternalState = statusDisplays.jcw.getInternalState();// Now remove the child.
ReactDOM.render(
<);
statusDisplays = parentInstance.getStatusDisplays();
e<FriendsStatusDisplay
{...props}
prepareChildren={prepareChildrenArray}
/>,
container,
...
s  });  it('should reset internal state if removed then readded in a legacy iterable', () => {
// Test basics.
const props = {
usernameToStatus: {
jcw: 'jcwStatus',
...
container,
);expect(statusDisplays.jcw).toBeFalsy();// Now reset the props that cause there to be a child
ReactDOM.render(
<FriendsStatusDisplay
{...props}
prepareChildren={prepareChildrenModernIterable}
...
{
usernuserThree: 'userThreeStatus',
userFour: 'userFourStatus',
userOne: 'userOneStatus',
},
},
...
const PROPS_SEQUENCE = [
{
usernjordanjcw: 'jordanjcwStatus',
