...
let waitForAll;describe('ReactDOMFizzServerHydrationWarning', () => {
  beforeEach(() => {
jest.resetModules();
...
<span>{'Client'}</span>
  ...
  });  // TODO: This behavior is not consistent with client-only single text node.  it('errors on server-only single text node mismatches with suppressHydrationWarning', async () => {
function App({isClient}) {
return (
<div>
<span suppressHydrationWarning={true}>
