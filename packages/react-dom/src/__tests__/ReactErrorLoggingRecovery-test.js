...
// It reproduces a combination of conditions that led to a problem.if (global.window) {
...
const ReactDOM = require('react-dom');// Initialize JSDOM separately.
...
global.navigator = jsdom.window.navigator;class Bad extends React.Component {
  componentDidUpdate() {
throw new Error('no');
  }
  render() {
return null;
  ...
  const originalConsoleError = console.error;  beforeEach(() => {
console.error = error => {
if (
typeof error === 'string' &&
...
expect(div.firstChild.textContent).toBe('Hello');// Verify the console.error bug is surfaced
expect(() => {
jest.runAllTimers();
