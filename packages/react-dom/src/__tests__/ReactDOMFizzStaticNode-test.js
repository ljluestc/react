...
let Suspense;describe('ReactDOMFizzStaticNode', () => {
  beforeEach(() => {
jest.resetModules();
React = require('react');
if (__EXPERIMENTAL__) {
...
expect(prelude).toContain('Loading');expect(errors).toEqual(['This operation was aborted']);
  });  // @gate experimental
...
controller.abort(theReason);const promise = ReactDOMFizzStatic.prerenderToNodeStreams(
<div>
<Suspense fallback={<div>Loading</div>}>
<InfiniteSuspend />
</Suspense>
...
});await jest.runAllTimers();controller.abort('foobar');await resultPromise;expect(errors).toEqual(['foobar', 'foobar']);
  });  // @gate experimental
...
});await jest.runAllTimers();controller.abort(new Error('uh oh'));await resultPromise;expect(errors).toEqual(['uh oh', 'uh oh']);
  });
});
