...
let waitForAll;
let waitFor;
let assertLog;describe('ReactUpdates', () => {
  beforeEach(() => {
jest.resetModules();
React = require('react');
...
let updateCount = 0;class Component extends React.Component {
state = {x: 0};componentDidUpdate() {
updateCount++;
}render() {
return <div>{this.state.x}</div>;
}
}const instance = ReactTestUtils.renderIntoDocument(<Component />);
expect(instance.state.x).toBe(0);ReactDOM.unstable_batchedUpdates(function () {
...
let updateCount = 0;class Component extends React.Component {
state = {x: 0, y: 0};componentDidUpdate() {
updateCount++;
}render() {
return (
<div>
...
expect(instance.state.x).toBe(0);
expect(instance.state.y).toBe(0);ReactDOM.unstable_batchedUpdates(function () {
instance.setState({x: 1});
...
let parentUpdateCount = 0;class Parent extends React.Component {
state = {x: 0};
childRef = React.createRef();componentDidUpdate() {
parentUpdateCount++;
}render() {
return (
<div>
...
}let childUpdateCount = 0;class Child extends React.Component {
state = {y: 0};componentDidUpdate() {
childUpdateCount++;
}render() {
...
expect(instance.state.x).toBe(0);
expect(child.state.y).toBe(0);ReactDOM.unstable_batchedUpdates(function () {
instance.setState({x: 1});
child.setState({y: 2});
expect(instance.state.x).toBe(0);
...
expect(child.state.y).toBe(2);
expect(parentUpdateCount).toBe(1);// Batching reduces the number of updates here to 1.
expect(childUpdateCount).toBe(1);
...
expect(instance.state.x).toBe(0);let innerCallbackRun = false;
...
let updateCount = 0;class Component extends React.Component {
state = {x: 0};shouldComponentUpdate() {
shouldUpdateCount++;
}componentDidUpdate() {
updateCount++;
}render() {
...
expect(instance.state.x).toBe(0);let callbacksRun = 0;
ReactDOM.unstable_batchedUpdates(function () {
instance.setState({x: 1}, function () {
callbacksRun++;
});
...
let childRenderCount = 0;class Parent extends React.Component {
childRef = React.createRef();shouldComponentUpdate() {
return false;
}render() {
parentRenderCount++;
...
expect(childRenderCount).toBe(0);let instance = <Parent />;
instance = ReactTestUtils.renderIntoDocument(instance);expect(parentRenderCount).toBe(1);
expect(childRenderCount).toBe(1);ReactDOM.unstable_batchedUpdates(function () {
instance.setState({x: 1});
...
let numBottomRenders = 0;class Top extends React.Component {
render() {
return (
<Middle>
<Bottom />
...
let willUpdates = [];
let didUpdates = [];const UpdateLoggingMixin = {
...
boxDivRef = React.createRef();render() {
return <div ref={this.boxDivRef}>{this.props.children}</div>;
}
}
Object.assign(Box.prototype, UpdateLoggingMixin);class Child extends React.Component {
spanRef = React.createRef();render() {
return <span ref={this.spanRef}>child</span>;
}
}
Object.assign(Child.prototype, UpdateLoggingMixin);class Switcher extends React.Component {
state = {tabKey: 'hello'};
boxRef = React.createRef();
switcherDivRef = React.createRef();
render() {
const child = this.props.children;return (
<Box ref={this.boxRef}>
<div
...
Object.assign(Switcher.prototype, UpdateLoggingMixin);class App extends React.Component {
...
Object.assign(App.prototype, UpdateLoggingMixin);let root = <App />;
root = ReactTestUtils.renderIntoDocument(root);function expectUpdates(desiredWillUpdates, desiredDidUpdates) {
let i;
...
let i;ReactDOM.unstable_batchedUpdates(function () {
for (i = 0; i < components.length; i++) {
triggerUpdate(components[i]);
}
});expectUpdates(desiredWillUpdates, desiredDidUpdates);// Try them in reverse orderReactDOM.unstable_batchedUpdates(function () {
...
// updated.const bContainer = document.createElement('div');let b;let aUpdated = false;class A extends React.Component {
state = {x: 0};componentDidUpdate() {
...
const updates = [];class Outer extends React.Component {
state = {x: 0};
innerRef = React.createRef();render() {
...
}const instance = ReactTestUtils.renderIntoDocument(<Outer />);updates.push('Outer-setState-1');
instance.setState({x: 1}, function () {
...
'Outer-render-0',
'Inner-render-0-0','Outer-setState-1',
'Outer-render-1',
'Inner-render-1-0',
'Inner-didUpdate-1-0',
'Outer-didUpdate-1',
...
'Inner-setState-1',
'Outer-callback-1',// Happens in a batch
'Outer-setState-2',// Flush batched updates all at once
'Outer-render-2',
'Inner-render-2-1',
'Inner-didUpdate-2-1',
'Inner-callback-1',
...
const instances = [];
const updates = [];class MockComponent extends React.Component {
render() {
...
}ReactTestUtils.renderIntoDocument(<MockComponent depth={0} count={2} />);expect(updates).toEqual([0, 1, 2]);ReactDOM.unstable_batchedUpdates(function () {
...
// See https://github.com/facebook/react/issues/1147class X extends React.Component {
state = {s: 0};render() {
if (this.state.s === 0) {
return (
<div>
...
expect(ReactDOM.findDOMNode(x).textContent).toBe('0');y.forceUpdate();
...
let a;class A extends React.Component {
state = {x: 0};UNSAFE_componentWillMount() {
a = this;
}render() {
...
let callbackCount = 0;class A extends React.Component {
state = {x: this.props.x};UNSAFE_componentWillReceiveProps(nextProps) {
...
let renderCount = 0;
let componentB = null;class B extends React.Component {
state = {updates: 0};componentDidMount() {
componentB = this;
}render() {
renderCount++;
return <div />;
...
state = {showB: true};render() {
return this.state.showB ? <B /> : <div />;
}
}const component = ReactTestUtils.renderIntoDocument(<A />);ReactDOM.unstable_batchedUpdates(function () {
...
}let component = ReactTestUtils.renderIntoDocument(<A />);expect(() => {
...
childRef = React.createRef();getChild = () => {
...
let once = false;class Child extends React.Component {
state = {updated: false};UNSAFE_componentWillUpdate() {
if (!once) {
once = true;
...
const ops = [];class Foo extends React.Component {
state = {step: 0};
render() {
...
const container = document.createElement('div');let instance;
let ops = [];
class Foo extends React.Component {
render() {
instance = this;
...
const container = document.createElement('div');function Baz() {
Scheduler.log('Baz');
return <p>baz</p>;
}let setCounter;
function Bar() {
...
expect(hiddenDiv.innerHTML).toBe('<p>bar 0</p>');// Run offscreen update
await waitForAll(['Bar']);
...
const container = document.createElement('div');
const ref = React.createRef();class EventuallyTerminating extends React.Component {
state = {step: 0};
componentDidMount() {
...
}).toThrow('Maximum');// Verify that we don't go over the limit if these updates are unrelated.
limit -= 10;
ReactDOM.render(<EventuallyTerminating ref={ref} />, container);
...
expect(container.textContent).toBe(limit.toString());
ref.current.setState({step: 0});
expect(container.textContent).toBe(limit.toString());limit += 10;
expect(() => {
ref.current.setState({step: 0});
}).toThrow('Maximum');
...
expect(() => {
ReactDOM.render(<NonTerminating />, container);
}).toThrow('Maximum');ReactDOM.render(<Terminating />, container);
expect(container.textContent).toBe('1');expect(() => {
ReactDOM.render(<NonTerminating />, container);
}).toThrow('Maximum');ReactDOM.render(<Terminating />, container);
expect(container.textContent).toBe('1');
  });  it('does not fall into mutually recursive infinite update loop with same container', () => {
// Note: this test would fail if there were two or more different roots.class A extends React.Component {
componentDidMount() {
...
this.setState({});this.props.parent.remount();
}
render() {
return <BadRender />;
}
...
const subscribers = [];class Child extends React.Component {
state = {value: 'initial'};
componentDidMount() {
...
ReactDOM.render(<App />, container);ReactDOM.unstable_batchedUpdates(() => {
...
let _setStep;
const LIMIT = 50;function Terminating() {
const [step, setStep] = React.useState(0);
_setStep = setStep;
React.useEffect(() => {
if (step < LIMIT) {
...
spyOnDev(console, 'error').mockImplementation(() => {});function NonTerminating() {
const [step, setStep] = React.useState(0);
React.useEffect(() => {
