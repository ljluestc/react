 ...
 *
 * @emails react-core
 */'use strict';describe('ReactMultiChild', () => {
  let React;
...
const container = document.createElement('div');const mockMount = jest.fn();
const mockUpdate = jest.fn();
const mockUnmount = jest.fn();class MockComponent extends React.Component {
...
expect(mockUnmount).toHaveBeenCalledTimes(0);ReactDOM.render(
<div>
<MockComponent />
</div>,
container,
...
expect(mockUnmount).toHaveBeenCalledTimes(0);ReactDOM.render(<WrapperComponent />, container);expect(mockMount).toHaveBeenCalledTimes(1);
expect(mockUnmount).toHaveBeenCalledTimes(0);ReactDOM.render(
<WrapperComponent>
<MockComponent />
</WrapperComponent>,
...
const container = document.createElement('div');class WrapperComponent extends React.Component {
render() {
...
}ReactDOM.render(<Parent>{[<div key="1" />]}</Parent>, container);expect(() =>
ReactDOM.render(
...
ReactDOM.render(<Foo />, div);
expect(div.textContent).toBe('HelloWorld');ReactDOM.render(<Foo />, div);
...
}const container = document.createElement('div');// Two random strings -- some additions, some removals, some moves
...
const SpyB = props => <Spy {...props} />;const container = document.createElement('div');
ReactDOM.render(
<div>
...
'twoA componentDidMount','oneB componentWillMount',
'oneB render',
'twoB componentWillMount',
'twoB render',
'oneA componentWillUnmount',
'twoA componentWillUnmount','oneB componentDidMount',
