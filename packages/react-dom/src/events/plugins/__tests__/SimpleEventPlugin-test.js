 ...
 *
 * @emails react-core
 */'use strict';describe('SimpleEventPlugin', function () {
  let React;
  let ReactDOM;
  let ReactDOMClient;
  let Scheduler;
  ...
  let container;
  let assertLog;
  let waitForAll;  function expectClickThru(element) {
element.click();
...
waitForAll = InternalTestUtils.waitForAll;onClick = jest.fn();
  });  afterEach(() => {
...
const child = element.querySelector('span');child.click();
expect(onClick).toHaveBeenCalledTimes(0);
...
container = document.createElement('div');
document.body.appendChild(container);let button;
...
assertLog([]);click();// There should be exactly one update.
...
Scheduler = require('scheduler');const InternalTestUtils = require('internal-test-utils');
assertLog = InternalTestUtils.assertLog;
waitForAll = InternalTestUtils.waitForAll;act = require('internal-test-utils').act;
...
// Flush async work
await waitForAll(['render button: enabled']);function click() {
...
// Flush async work
await waitForAll([]);
expect(button.textContent).toEqual('Count: 0');function click() {
const event = new MouseEvent('click', {
bubbles: true,
cancelable: true,
});
...
expect(button.textContent).toEqual('Count: 1');// Click the button many more times
await act(() => click());
await act(() => click());
await act(() => click());
await act(() => click());
await act(() => click());
await act(() => click());// Flush the remaining work
await waitForAll([]);
...
// See http://www.quirksmode.org/blog/archives/2010/09/click_event_del.htmlit('does not add a local click to interactive elements', function () {
container = document.createElement('div');ReactDOM.render(<button onClick={onClick} />, container);const node = container.firstChild;node.dispatchEvent(new MouseEvent('click'));expect(onClick).toHaveBeenCalledTimes(0);
});it('adds a local click listener to non-interactive elements', function () {
container = document.createElement('div');ReactDOM.render(<div onClick={onClick} />, container);const node = container.firstChild;node.dispatchEvent(new MouseEvent('click'));expect(onClick).toHaveBeenCalledTimes(0);
});it('registers passive handlers for events affected by the intervention', () => {
container = document.createElement('div');const passiveEvents = [];
...
};ReactDOM.render(<div />, container);expect(passiveEvents).toEqual([
'touchstart',
'touchstart',
'touchmove',
'touchmove',
