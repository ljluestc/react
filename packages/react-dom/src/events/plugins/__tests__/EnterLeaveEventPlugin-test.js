...
let ReactDOM;describe('EnterLeaveEventPlugin', () => {
  let container;  beforeEach(() => {
jest.resetModules();React = require('react');
ReactDOM = require('react-dom');// The container has to be attached for events to fire.
...
iframeDocument.close();const leaveEvents = [];
...
iframeDocument.close();const enterEvents = [];
const node = ReactDOM.render(
<div
onMouseEnter={e => {
e.persist();
...
let parent = null;class Parent extends React.Component {
render() {
return (
<div
...
ReactDOM.render(<Parent showChild={true} />, container);// Enter from parent into the child.
...
const mockFn = jest.fn();class Parent extends React.Component {
constructor(props) {
super(props);
...
}ReactDOM.render(<Component />, container);// Leave from the portal div
