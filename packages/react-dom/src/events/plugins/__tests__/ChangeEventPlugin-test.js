...
let waitForAll;
let waitForDiscrete;
let assertLog;const setUntrackedChecked = Object.getOwnPropertyDescriptor(
  HTMLInputElement.prototype,
  'checked',
).set;const setUntrackedValue = Object.getOwnPropertyDescriptor(
  HTMLInputElement.prototype,
  'value',
).set;const setUntrackedTextareaValue = Object.getOwnPropertyDescriptor(
  HTMLTextAreaElement.prototype,
  'value',
).set;describe('ChangeEventPlugin', () => {
...
assertLog = InternalTestUtils.assertLog;container = document.createElement('div');
...
container = null;
  ...
  // See https://github.com/facebook/react/pull/5746.  it('should consider initial text value to be current', () => {
let called = 0;function cb(e) {
called++;
...
node.dispatchEvent(new Event('change', {bubbles: true, cancelable: true}));// There should be no React change events because the value stayed the same.
...
expect(called).toBe(1);expect(node.checked).toBe(true);
node.dispatchEvent(
new MouseEvent('click', {bubbles: true, cancelable: true}),
);
expect(node.checked).toBe(false);
...
expect(called).toBe(0);// However, we can simulate user typing by calling the underlying setter.
...
expect(called).toBe(1);// Verify again that extra events without real changes are ignored.
input.dispatchEvent(new Event('input', {bubbles: true, cancelable: true}));
...
// (also true).
expect(called).toBe(0);// However, simulating a normal click should fire a React event because the
// real value (false) would have changed from the last tracked value (true).
input.click();
expect(called).toBe(1);
  });  it('should unmount', () => {
const input = ReactDOM.render(<input />, container);ReactDOM.unmountComponentAtNode(container);
...
const option2 = div.childNodes[1];// Select first option.
option1.click();
expect(called1).toBe(1);
expect(called2).toBe(0);// Select second option.
option2.click();
expect(called1).toBe(1);
expect(called2).toBe(1);// Select the first option.
...
expect(called).toBe(1);
ReactDOM.unmountComponentAtNode(container);called = 0;
input = ReactDOM.render(<input type={type} onChange={cb} />, container);
// Should be ignored (no change):
input.dispatchEvent(
...
input.dispatchEvent(new Event('input', {bubbles: true, cancelable: true}));setUntrackedValue.call(input, 20);
input.dispatchEvent(new Event('change', {bubbles: true, cancelable: true}));expect(called).toBe(2);
...
input.dispatchEvent(new Event('input', {bubbles: true, cancelable: true}));
input.dispatchEvent(new Event('change', {bubbles: true, cancelable: true}));setUntrackedValue.call(input, 'foo');
input.dispatchEvent(new Event('input', {bubbles: true, cancelable: true}));
input.dispatchEvent(new Event('change', {bubbles: true, cancelable: true}));expect(called).toBe(2);
...
let input;class ControlledInput extends React.Component {
state = {value: 'initial'};
...
expect(input.value).toBe('initial');// Trigger a change event.
...
expect(input.checked).toBe(false);// Trigger a change event.
input.dispatchEvent(
new MouseEvent('click', {bubbles: true, cancelable: true}),
);
// Change should synchronously flush
assertLog(['render: true']);
expect(input.checked).toBe(true);// Now let's make sure we're using the controlled value.
root.render(<ControlledInput reverse={true} />);
await waitForAll(['render: true']);// Trigger another change event.
input.dispatchEvent(
new MouseEvent('click', {bubbles: true, cancelable: true}),
);
// Change should synchronously flush
...
const root = ReactDOMClient.createRoot(container);
let textarea;class ControlledTextarea extends React.Component {
state = {value: 'initial'};
onChange = event => this.setState({value: event.target.value});
render() {
Scheduler.log(`render: ${this.state.value}`);
...
// Flush callbacks.
await waitForAll(['render: initial']);
expect(textarea.value).toBe('initial');// Trigger a change event.
...
// Flush callbacks.
await waitForAll(['render: initial']);
expect(input.value).toBe('initial');// Trigger a click event
input.dispatchEvent(
...
const {useState} = React;const root = ReactDOMClient.createRoot(container);const target = React.createRef(null);
function Foo() {
...
expect(container.textContent).toEqual('not hovered');await act(() => {
const mouseOverEvent = document.createEvent('MouseEvents');
mouseOverEvent.initEvent('mouseover', true, true);
target.current.dispatchEvent(mouseOverEvent);// Flush discrete updates
ReactDOM.flushSync();
