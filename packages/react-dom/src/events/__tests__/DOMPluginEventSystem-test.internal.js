 ...
 *
 * @emails react-core
 */'use strict';import {createEventTarget} from 'dom-event-testing-library';let React;
...
let act;
let waitForAll;
let waitFor;function dispatchEvent(element, type) {
  const event = document.createEvent('Event');
  event.initEvent(type, true, true);
  element.dispatchEvent(event);
}function dispatchClickEvent(element) {
  dispatchEvent(element, 'click');
}const eventListenersToClear = [];function startNativeEventListenerClearDown() {
...
target.removeEventListener(...params);
  ...
  let container;  function withEnableLegacyFBSupport(enableLegacyFBSupport) {
describe(
...
beforeAll(() => {
  ...
  ReactFeatureFlags.enableLegacyFBSupport = enableLegacyFBSupport;  React = require('react');
  ReactDOM = require('react-dom');
  ReactDOMClient = require('react-dom/client');
  Scheduler = require('scheduler');
  ReactDOMServer = require('react-dom/server');  const InternalTestUtils = require('internal-test-utils');
  waitForAll = InternalTestUtils.waitForAll;
  waitFor = InternalTestUtils.waitFor;
  act = InternalTestUtils.act;  container = document.createElement('div');
  ...
  const log = [];
  const onClick = jest.fn(e => log.push(e));  function Test() {
return <button ref={buttonRef} onClick={onClick} />;
  }  ReactDOM.render(<Test />, container);  const buttonElement = buttonRef.current;
...
<div
  ...
  expect(log[1]).toEqual(['bubble', buttonElement]);  const divElement = divRef.current;
...
<div ref={childRef} />
  ...
  ReactDOM.render(<Child />, childRef.current);  const buttonElement = buttonRef.current;
  dispatchClickEvent(buttonElement);
  expect(onClick).toHaveBeenCalledTimes(1);
  expect(onClickCapture).toHaveBeenCalledTimes(1);
  expect(log[0]).toEqual(['capture', buttonElement]);
...
onClickCapture={onClickCapture}
  ...
  ReactDOM.render(<Child />, disjointedNode);  const buttonElement = buttonRef.current;
  dispatchClickEvent(buttonElement);
  expect(onClick).toHaveBeenCalledTimes(1);
  expect(onClickCapture).toHaveBeenCalledTimes(1);
  expect(log[0]).toEqual(['capture', buttonElement]);
...
<span ref={spanRef} />
  ...
  // - container
  // - parentContainer
  // - childContainer  const parentContainer = document.createElement('div');
  const childContainer = document.createElement('div');  ReactDOM.render(<GrandParent />, container);
  ReactDOM.render(<Parent />, parentContainer);
  ReactDOM.render(<Child />, childContainer);  parentContainer.appendChild(childContainer);
  spanRef.current.appendChild(parentContainer);  // Inside <GrandParent />
  const buttonElement = buttonRef.current;
  dispatchClickEvent(buttonElement);
  expect(onClick).toHaveBeenCalledTimes(1);
  expect(onClickCapture).toHaveBeenCalledTimes(1);
  expect(log[0]).toEqual(['capture', buttonElement]);
  expect(log[1]).toEqual(['bubble', buttonElement]);  // Inside <Child />
  ...
  expect(log[3]).toEqual(['capture', divElement]);
  expect(log[4]).toEqual(['bubble', divElement]);
  expect(log[5]).toEqual(['bubble', buttonElement]);  // Inside <Parent />
...
document.body.appendChild(portalElement);function Child() {
return (
<div
 ref={divRef}
 onClick={onClick}
...
}ReactDOM.render(<Parent />, container);const buttonElement = buttonRef.current;
dispatchClickEvent(buttonElement);
expect(onClick).toHaveBeenCalledTimes(1);
expect(onClickCapture).toHaveBeenCalledTimes(1);
expect(log[0]).toEqual(['capture', buttonElement]);
...
expect(log[3]).toEqual(['capture', divElement]);
expect(log[4]).toEqual(['bubble', divElement]);
expect(log[5]).toEqual(['bubble', buttonElement]);document.body.removeChild(portalElement);
});it('handle click events on document.body portals', () => {
const log = [];function Child({label}) {
...
dispatchClickEvent(second);expect(log).toEqual(['second']);const first = second.previousSibling;
...
});childSlotRef.current.appendChild(childContainer);childContainer.innerHTML = finalHTML;const a = childContainer.getElementsByTagName('a')[0];suspend = true;// Hydrate asynchronously.
await act(() => {
...
const log = [];function Parent() {
const ref = React.useRef(null);
const [portal, setPortal] = React.useState(null);React.useEffect(() => {
setPortal(
 ReactDOM.createPortal(
...
expect(parent.id).toEqual('parent');await act(() => {
dispatchClickEvent(parent);
});expect(log).toEqual(['parent']);const child = parent.lastChild;
expect(child.id).toEqual('child');await act(() => {
...
dispatchClickEvent(divElement);
expect(onClick).toHaveBeenCalledTimes(1);
expect(onClickCapture).toHaveBeenCalledTimes(3);document.body.removeChild(portalElement);
...
divElement.focus();
expect(onFocus).toHaveBeenCalledTimes(1);
expect(onFocusCapture).toHaveBeenCalledTimes(3);document.body.removeChild(portalElement);
});it('handle propagation of enter and leave events between portals', () => {
const buttonRef = React.createRef();
const divRef = React.createRef();
const log = [];
const onMouseEnter = jest.fn(e => log.push(e.currentTarget));
const onMouseLeave = jest.fn(e => log.push(e.currentTarget));const portalElement = document.createElement('div');
document.body.appendChild(portalElement);function Child() {
return (
<div
 ref={divRef}
...
expect(log[0]).toEqual(buttonElement);const divElement = divRef.current;
buttonElement.dispatchEvent(
...
expect(onMouseLeave).toHaveBeenCalledTimes(0);
expect(log[1]).toEqual(divElement);document.body.removeChild(portalElement);
...
const log = [];
const onMouseEnter = jest.fn(e => log.push(e.currentTarget));
const onMouseLeave = jest.fn(e => log.push(e.currentTarget));function Child() {
return (
<div
 ref={divRef}
 onMouseEnter={onMouseEnter}
...
const [portal, setPortal] = React.useState(null);React.useLayoutEffect(() => {
...
const onClickCapturePortal = jest.fn(e => log.push('capture portal'));function Portal() {
return (
<div
...
}const portalContainer = document.createElement('div');let shouldStopPropagation = false;
...
}ReactDOM.render(<Root />, container);const divElement = targetRef.current;
dispatchClickEvent(divElement);
expect(log).toEqual([
'capture root',
'capture portal',
'bubble portal',
'bubble root',
]);log = [];shouldStopPropagation = true;
dispatchClickEvent(divElement);if (enableLegacyFBSupport) {
...
act = require('internal-test-utils').act;const InternalTestUtils = require('internal-test-utils');
waitForAll = InternalTestUtils.waitForAll;
waitFor = InternalTestUtils.waitFor;
});// @gate www
it('can render correctly with the ReactDOMServer', () => {
const clickEvent = jest.fn();
const setClick = ReactDOM.unstable_createEventHandle('click');function Test() {
const divRef = React.useRef(null);React.useEffect(() => {
...
expect(clickEvent).toBeCalledTimes(1);// Unmounting the container and clicking should not work
await act(() => {
ReactDOM.render(null, container);
});dispatchClickEvent(divElement);
expect(clickEvent).toBeCalledTimes(1);// Re-rendering the container and clicking should work
await act(() => {
ReactDOM.render(<Test />, container);
});divElement = divRef.current;
dispatchClickEvent(divElement);
expect(clickEvent).toBeCalledTimes(2);log = [];// Clicking the button should also work
const buttonElement = buttonRef.current;
dispatchClickEvent(buttonElement);
expect(log).toEqual([
{
...
]);const setClick2 = ReactDOM.unstable_createEventHandle('click');function Test2({clickEvent2}) {
React.useEffect(() => {
...
dispatchClickEvent(divElement);
expect(clickEvent2).toBeCalledTimes(1);// Reset the function we pass in, so it's different
clickEvent2 = jest.fn();
await act(() => {
ReactDOM.render(<Test2 clickEvent2={clickEvent2} />, container);
});divElement = divRef.current;
...
const divRef = React.createRef();
const buttonRef = React.createRef();
const setClick = ReactDOM.unstable_createEventHandle('click');function Test({off}) {
React.useEffect(() => {
...
dispatchClickEvent(divElement);
expect(clickEvent).toBeCalledTimes(1);// The listener should get unmounted
await act(() => {
ReactDOM.render(<Test off={true} />, container);
});clickEvent.mockClear();divElement = divRef.current;
dispatchClickEvent(divElement);
...
expect(onClickCapture).toHaveBeenCalledTimes(1);
expect(log[0]).toEqual(['capture', buttonElement]);
expect(log[1]).toEqual(['bubble', buttonElement]);log.length = 0;
onClick.mockClear();
onClickCapture.mockClear();const divElement = divRef.current;
dispatchClickEvent(divElement);
expect(onClick).toHaveBeenCalledTimes(2);
...
React.useEffect(() => {
 ...
 setClickCapture(buttonRef.current, onClickCapture); return () => {
setClick();
setClickCapture();
 };
});return (
...
dispatchClickEvent(divElement);
expect(clickEvent).toBeCalledTimes(2);// Clicking the button should not work
const buttonElement = buttonRef.current;
dispatchClickEvent(buttonElement);
...
let setClick4 = ReactDOM.unstable_createEventHandle('click');function Test() {
React.useEffect(() => {
...
expect(targetListener4).toHaveBeenCalledTimes(1);setClick1 = ReactDOM.unstable_createEventHandle('click');
setClick2 = ReactDOM.unstable_createEventHandle('click');
setClick3 = ReactDOM.unstable_createEventHandle('click');
setClick4 = ReactDOM.unstable_createEventHandle('click');function Test2() {
React.useEffect(() => {
 const clearClick1 = setClick1(
buttonRef.current,
targetListener1,
...
const setClick2 = ReactDOM.unstable_createEventHandle('click');function Test() {
React.useEffect(() => {
...
const setClick4 = ReactDOM.unstable_createEventHandle('click');function Test() {
React.useEffect(() => {
 const clearClick1 = setClick1(
buttonRef.current,
targetListener1,
...
const setClick1 = ReactDOM.unstable_createEventHandle('click');function Test({counter}) {
React.useLayoutEffect(() => {
...
root.render(<Test counter={0} />);await waitForAll(['Test']);// Click the button
dispatchClickEvent(ref.current);
expect(log).toEqual([{counter: 0}]);// Clear log
log.length = 0;// Increase counter
React.startTransition(() => {
root.render(<Test counter={1} />);
});
// Yield before committing
await waitFor(['Test']);// Click the button again
dispatchClickEvent(ref.current);
expect(log).toEqual([{counter: 0}]);// Clear log
log.length = 0;// Commit
await waitForAll([]);
...
const setClick1 = ReactDOM.unstable_createEventHandle('click');function Test() {
React.useEffect(() => {
...
expect(clickEvent).toBeCalledTimes(1);// Re-rendering and clicking the body should work again
await act(() => {
ReactDOM.render(<Test />, container);
});dispatchClickEvent(document.body);
expect(clickEvent).toBeCalledTimes(2);
...
expect(log[3]).toEqual(['bubble', window]);log.length = 0;
onClick.mockClear();
onClickCapture.mockClear();const divElement = divRef.current;
dispatchClickEvent(divElement);
expect(onClick).toHaveBeenCalledTimes(3);
...
React.useEffect(() => {
 ...
 const clearClick4 = setClick4(window, rootListener4); return () => {
clearClick1();
clearClick2();
clearClick3();
clearClick4();
...
dispatchClickEvent(buttonElement);
expect(onClick).toHaveBeenCalledTimes(3);
expect(onClickCapture).toHaveBeenCalledTimes(3);if (enableLegacyFBSupport) {
expect(log[0]).toEqual(['capture', window]);
...
expect(onClickCapture).toHaveBeenCalledTimes(4);if (enableLegacyFBSupport) {
expect(log[0]).toEqual(['capture', window]);
expect(log[1]).toEqual(['capture', document]);
expect(log[2]).toEqual(['capture', buttonElement]);
expect(log[3]).toEqual(['capture', divElement]);
...
ReactDOM.unstable_createEventHandle('beforeblur');const Component = ({show}) => {
const ref = React.useRef(null);React.useEffect(() => {
 const clear1 = setAfterBlurHandle(document, onAfterBlur);
 const clear2 = setBeforeBlurHandle(ref.current, onBeforeBlur); return () => {
clear1();
clear2();
 };
});return (
...
expect(onAfterBlur).toHaveBeenCalledTimes(0);await act(() => {
...
const ref2 = React.createRef();const Component = ({show}) => {
const ref = React.useRef(null);React.useEffect(() => {
...
const target = createEventTarget(inner);
target.focus();
expect(onBeforeBlur).toHaveBeenCalledTimes(0);await act(() => {
ReactDOM.render(<Component show={false} />, container);
});expect(onBeforeBlur).toHaveBeenCalledTimes(1);
});// @gate www
...
ReactDOM.unstable_createEventHandle('afterblur');
const setBeforeBlurHandle =
ReactDOM.unstable_createEventHandle('beforeblur');function Child() {
if (suspend) {
 throw promise;
} else {
...
document.body.appendChild(container2);const root = ReactDOMClient.createRoot(container2);await act(() => {
root.render(<Component />);
});
jest.runAllTimers();const inner = innerRef.current;
const target = createEventTarget(inner);
target.focus();
expect(onBeforeBlur).toHaveBeenCalledTimes(0);
expect(onAfterBlur).toHaveBeenCalledTimes(0);suspend = true;
await act(() => {
root.render(<Component />);
});
jest.runAllTimers();expect(onBeforeBlur).toHaveBeenCalledTimes(1);
expect(onAfterBlur).toHaveBeenCalledTimes(1);
expect(onAfterBlur).toHaveBeenCalledWith(
expect.objectContaining({relatedTarget: inner}),
);
resolve();
expect(log).toEqual(['beforeblur', 'afterblur']);document.body.removeChild(container2);
});// @gate www
...
const target = createEventTarget(inner);
target.focus();
expect(onBeforeBlur).toHaveBeenCalledTimes(0);suspend = true;
await act(() => {
root.render(<Component />);
});
jest.runAllTimers();expect(onBeforeBlur).toHaveBeenCalledTimes(1);
...
const setBeforeBlurHandle =
ReactDOM.unstable_createEventHandle('beforeblur');
const innerRef = React.createRef();function Child() {
if (suspend) {
 throw fakePromise;
}
return <input ref={innerRef} />;
...
const ref = React.useRef(null);
const [, setState] = React.useState(0);React.useEffect(() => {
...
document.body.appendChild(container2);const root = ReactDOMClient.createRoot(container2);
await act(() => {
root.render(<Component />);
});// Focus the input node
const inner = innerRef.current;
const target = createEventTarget(inner);
target.focus();// Suspend. This hides the input node, causing it to lose focus.
suspend = true;
await act(() => {
root.render(<Component />);
...
expect(onClickCapture).toHaveBeenCalledTimes(1);
expect(log[0]).toEqual(['capture', buttonElement]);
expect(log[1]).toEqual(['bubble', buttonElement]);document.body.removeChild(portalElement);
});describe('Compatibility with Scopes API', () => {
beforeEach(() => {
jest.resetModules();
ReactFeatureFlags = require('shared/ReactFeatureFlags');
ReactFeatureFlags.enableCreateEventHandleAPI = true;
ReactFeatureFlags.enableScopeAPI = true;React = require('react');
ReactDOM = require('react-dom');
ReactDOMClient = require('react-dom/client');
Scheduler = require('scheduler');
ReactDOMServer = require('react-dom/server');
...
capture: true,
 ...
 const scopeRef = React.useRef(null); React.useEffect(() => {
...
const setClick = ReactDOM.unstable_createEventHandle('click');function Test() {
 ...
 const scope2Ref = React.useRef(null); React.useEffect(() => {
const clear1 = setClick(scopeRef.current, outerOnClick);
const clear2 = setClick(scope2Ref.current, innerOnClick);return () => {
clear1();
clear2();
};
 }); return (
...
const setClick = ReactDOM.unstable_createEventHandle('click');function Test() {
 ...
 const scope2Ref = React.useRef(null); React.useEffect(() => {
const clear1 = setClick(scopeRef.current, onClick);
const clear2 = setClick(scope2Ref.current, onClick);return () => {
clear1();
clear2();
};
 }); return (
...
container.appendChild(rootContainer);const allEvents = [];
...
container.addEventListener('wheel', handler);const ref = React.createRef();
const setTouchStart =
 ReactDOM.unstable_createEventHandle('touchstart');
const setTouchMove =
 ReactDOM.unstable_createEventHandle('touchmove');
const setWheel = ReactDOM.unstable_createEventHandle('wheel');function Component() {
 React.useEffect(() => {
...
dispatchEvent(ref.current, 'wheel');expect(allEvents).toEqual(['touchstart', 'touchmove', 'wheel']);
