 ...
 * @flow
 */
'use strict';  target: Node,
  CSPnonce: string | null,
) {
  const ownerDocument = target.ownerDocument || target;  // We need to remove the script content for any scripts that would not run based on CSP
  // We restore the script content after moving the nodes into the target
  const badNonceScriptNodes: Map<Element, string> = newconst scripts = source.querySelectorAll('script');
...
lastChild = node;if (node.nodeType === 1) {
const element: Element = (node: any);
if (
// $FlowFixMe[prop-missing]
element.dataset != nuelement.dataset.rri != null ||
...
target.appendChild(element);if (element.nodeName === 'SCRIPT') {
