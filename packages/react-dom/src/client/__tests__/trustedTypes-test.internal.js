 ...
 *
 * @emails react-core
 */'use strict';describe('when Trusted Types are available in global object', () => {
  let React;
  let ReactDOM;
  let ReactFeatureFlags;
  let container;
  let ttObject1;
  let ttObject2;  beforeEach(() => {
jest.resetModules();
...
container,
  ...
  expect(innerHTMLCalls[0]).toBe(ttObject1);  innerHTMLCalls.length = 0;
  ReactDOM.render(
...
return setAttribute.apply(this, arguments);
  ...
  // Ensure it didn't get stringified when passed to a DOM sink:
  expect(setAttributeCalls[0][2]).toBe(ttObject1);  setAttributeCalls.length = 0;
...
return setAttributeNS.apply(this, arguments);
  ...
  // Ensure it didn't get stringified when passed to a DOM sink:
  expect(setAttributeNSCalls[0][3]).toBe(ttObject1);  setAttributeNSCalls.length = 0;
...
let innerHTMLDescriptor;// simulate svg elements in Internet Explorer which don't have 'innerHTML' property
beforeEach(() => {
