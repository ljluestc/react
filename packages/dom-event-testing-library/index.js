 ...
 *
 * @emails react-core
 */'use strict';import {buttonType, buttonsType} from './constants';
...
import {describeWithPointerEvent, testWithPointerType} from './testHelpers';const createEventTarget = node => ({
  node,
  /**
imple events abstraction.
...
node.dispatchEvent(domEvents.virtualclick(payload));
  ...
  /**
ointerEvent abstraction.
ispatches the expected sequence of PointerEvents, MouseEvents, and
ouchEvents for a given environment.
...
domEventSequences.pointerup(node, payload);
  ...
  /**
esture abstractions.
elpers for event sequences expected in a gesture.
arget.tap({ pointerType: 'touch' })
...
});const resetActivePointers = domEventSequences.resetActivePointers;export {
  buttonType,
  buttonsType,
  createEventTarget,
