 ...
 */'use strict';/* eslint-disable no-for-of-loops/no-for-of-loops */// Hi, if this is your first time editing/reading a Dangerfile, here's a summary:
...
// So, for example:
//
// `DANGER_GITHUB_API_TOKEN=[ENV_ABOVE] yarn danger pr https://github.com/facebook/react/pull/11865const {markdown, danger, warn} = require('danger');
const {promisify} = require('util');
const glob = promisify(require('glob'));
const gzipSize = require('gzip-size');const {readFileSync, statSync} = require('fs');const BASE_DIR = 'base-build';
const HEAD_DIR = 'build';const CRITICAL_THRESHOLD = 0.02;
...
`${kbs(result.headSizeGzip)}`,
  ...
  // where the branches differ  const upstreamRepo = danger.github.pr.base.repo.full_name;
...
return;  const resultsMap = new Map();  // Find all the head (current) artifacts paths.
...
const baseSizeGzip = gzipSize.fileSync(BASE_DIR + '/' + artifactPath);const headSize = statSync(HEAD_DIR + '/' + artifactPath).size;
...
resultsMap.set(artifactPath, {
 ...
 changeGzip: -1,
  ...
  results.sort((a, b) => b.change - a.change);  let criticalResults = [];
...
Comparing: ${baseSha}...${headSha}## Critical size changesIncludes critical production bundles, as well as any change greater than ${
